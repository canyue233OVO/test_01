...
public class ClientProxy extends CommonProxy
{
    ...
    public void load()
    {
        ...
        //FMLCommonHandler.instance().bus().register(new PlaneHUD());
    ...
    public void doTutorialStuff(EntityPlayer player, EntityDriveable entityType)
    {
        //if (!FlansModClient.doneTutorial)
        {
            ...
            player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.inventoryKey.getKeyCode()) + " 打开菜单"));
            player.addChatComponentMessage(new ChatComponentText("按住 " + Keyboard.getKeyName(Minecraft.getMinecraft().gameSettings.keyBindSneak.getKeyCode()) + " 下车"));
            player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.eject.getKeyCode()) + " 弹出乘客并启用驾驶员控制的乘客武器"));
            player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.openBorders.getKeyCode()) + " 再次允许乘客和乘客控制的武器"));
            ...
            {
                player.addChatComponentMessage(new ChatComponentText("按住 " + Keyboard.getKeyName(KeyInputHandler.nintendoSwitch.getKeyCode()) + " 切换座位"));
                player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.controlSwitchKey.getKeyCode()) + " 启用鼠标控制模式"));
                if(PlaneType.getPlane(((EntityPlane)entityType).driveableType).hasGear)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.gearKey.getKeyCode()) + " 切换起落架"));
                if(PlaneType.getPlane(((EntityPlane)entityType).driveableType).hasDoor)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.doorKey.getKeyCode()) + " 切换舱门"));
                if(PlaneType.getPlane(((EntityPlane)entityType).driveableType).hasAfterBurner)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.trimKey.getKeyCode()) + " 切换后燃器"));
                if(PlaneType.getPlane(((EntityPlane)entityType).driveableType).mode == EnumPlaneMode.VTOL)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.modeKey.getKeyCode()) + " 切换垂直起降模式"));
                    ...
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.modeKey.getKeyCode()) + " 切换着陆襟翼"));
                if(PlaneType.getPlane(((EntityPlane)entityType).driveableType).carrierLandable || PlaneType.getPlane(((EntityPlane)entityType).driveableType).helipadLandable)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.gunKey.getKeyCode()) + " 在机坪/水上机库对接"));
                if(entityType.getDriveableType().hasMagicArtilleryMode)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.artilleryMode.getKeyCode()) + " 切换魔法弹道视觉"));
            }
            if (entityType instanceof EntityVehicle)
            {
                ...
                //    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.leftRollKey.getKeyCode()) + " 锁定坦克炮塔/火炮"));
                //if(((EntityVehicle) entityType).getVehicleType().tank)
                //    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.rightRollKey.getKeyCode()) + " 解锁坦克炮塔/火炮"));
                if(((EntityVehicle) entityType).getVehicleType().thermalSight)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.thermal.getKeyCode()) + " 切换热成像瞄准镜"));
                if(entityType.getDriveableType().hasMagicArtilleryMode)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.artilleryMode.getKeyCode()) + " 切换魔法弹道视觉"));        
                if(entityType.getDriveableType().hasRadar)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.artilleryMode.getKeyCode()) + " 切换主动/被动雷达"));            
                if(entityType.getDriveableType().hasScope)
                    player.addChatComponentMessage(new ChatComponentText("按 " + Keyboard.getKeyName(KeyInputHandler.downKey.getKeyCode()) + " 切换武器瞄准镜"));
                player.addChatComponentMessage(new ChatComponentText("按住空格键刹车"));
            }
        }
    }
    ...
    public void changeControlMode(EntityPlayer player)
    {
        if(FlansModClient.flipControlMode())
            player.addChatComponentMessage(new ChatComponentText("鼠标控制模式已设置为 " + FlansModClient.controlModeMouse));
    }
    ...
    private static String getModelName(String in)
    {
        //Split about dots
        ...
        if (mc.renderViewEntity != null && mc.effectRenderer != null)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //2025 ripoff of that other mod
                    if (p_72726_1_.equals("flansmod.NuFlash"))
                    {
                        entityfx = new EntityNuFlash(theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_);
                    }
                    
                    if (p_72726_1_.equals("flansmod.NuMuzzle"))
                    {
                        entityfx = new EntityNuMuzzle(theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_);
                    }
                    
                    if (p_72726_1_.equals("flansmod.TurretTest"))
                    {
                        entityfx = new EntityTurretTest(theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_);
                    }
                    
                    if (p_72726_1_.equals("flansmod.NuSpark"))
                    {
                        entityfx = new EntityNuSpark(theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_);
                    }
