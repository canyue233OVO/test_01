			...
			else
			{
				...
				else if(bullet.truePen<this.armor && (0.9*(this.armor))<bullet.truePen && !bullet.isHEAT)
				{
					...
					if (ouch > 2*this.maxHealth && hit.driveable.epicShip && this.maxHealth > 1) //check if overkill damage to intact part
					{
						...
						{
							hit.driveable.getDriveableData().parts.get(EnumDriveablePart.bow).health -= 0.4*ouch; //do damage to main ship parts
							hit.driveable.getDriveableData().parts.get(EnumDriveablePart.left).health -= 0.4*ouch; //do damage to main ship parts
							hit.driveable.getDriveableData().parts.get(EnumDriveablePart.right).health -= 0.4*ouch; //do damage to main ship parts
							hit.driveable.getDriveableData().parts.get(EnumDriveablePart.stern).health -= 0.4*ouch; //do damage to main ship parts
							if(bullet != null)
				...
				//test exp 2024 baris
				{
				//	System.out.println("attempting baris tech");
			        BarisTechTreeHandler.BarisPacket packet = new BarisPacket();
			        packet.add("method", "addXP");
			        packet.add("type", "vehicle");
			        packet.add("amount", 1000);
			        packet.add("player", bullet.owner.getCommandSenderName());
