...
public class ItemGrenade extends ItemShootable implements IFlanItem
{
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player)
	{
		...
		if(type.canThrow && data != null && data.shootTimeRight <= 0 && data.shootTimeLeft <= 0 && TeamsManager.violence==false)
		{
			...
			EntityGrenade grenade = new EntityGrenade(world, type, player, null);
	...
	{
		return getGrenade(worldObj, thrower, null);
	}
	...
	{
		return getGrenade(worldObj, player, null);
	}
	
	public EntityGrenade getGrenade(World world, EntityLivingBase thrower, EntityDriveable master)
	{
		//Create a new grenade entity
		EntityGrenade grenade = new EntityGrenade(world, type, thrower, master);
		//If this can be remotely detonated, add it to the players detonate list
