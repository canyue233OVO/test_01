...
public class GuiDriveableMenu extends GuiContainer
{
    ...
    {
		boolean planeInventoryNotAllowed = (entity instanceof EntityPlane && !inventory.player.capabilities.isCreativeMode  && (  ((EntityPlane)entity).mode == EnumPlaneMode.PLANE || ((EntityPlane)entity).mode == EnumPlaneMode.VTOL)   && (  ((EntityPlane)entity).control.V > 0.2f*((EntityPlane)entity).getPlaneType().takeoffSpeed || entity.throttle > 0.1f)  );
        ...
        if(button.id == 1) //Guns
        {
        	if(planeInventoryNotAllowed)
        		inventory.player.addChatMessage(new ChatComponentText("Your plane is too fast to access ammunition now"));
        	else
