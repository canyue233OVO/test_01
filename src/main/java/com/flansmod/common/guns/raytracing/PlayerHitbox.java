...
public class PlayerHitbox 
{
	...
	public float hitByBullet(EntityBullet bullet, float penetratingPower) 
	{
		...
		boolean disarmed = false; //for variable gore physics
		boolean myLeg = false; //for variable gore physics
		boolean decapitated = false; //for variable gore physics
...
decapitated = true;
		...
		case BODY : case BACK :  case NAPE :  case FACE : case LEFTARM :  case RIGHTARM :  case LEGS : case RIGHTITEM : case CRANIUM :	case LEFTITEM : 
		{
			...
			if (hitDamage > 40 && !player.getEntityData().getString("lastClowder").equals("SAFEZONE"))
			{
				...
				if (hitDamage>123)	 //july 1 2023 overkill sounds
				{
				PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 15, player.dimension, "deathOverkillLong", false); //123+ damage play the penis in power outlet 13 second super scream
				
				
				ItemStack armStack = InfoType.getRecipeElement("goreArm", 1); //spawn arm for physics corpse
				Item arm = armStack.getItem();
				ItemStack legStack = InfoType.getRecipeElement("goreLeg", 1); //spawn leg
				Item leg = legStack.getItem();
				ItemStack headStack = InfoType.getRecipeElement("goreHead", 1); //spawn head
				Item head = headStack.getItem();
				if (leg != null && leg instanceof ItemGrenade && head != null && head instanceof ItemGrenade && arm != null && arm instanceof ItemGrenade && !player.capabilities.isCreativeMode) //no infinite spam by shooting admins
				{
				EntityGrenade Arm = new EntityGrenade(player.worldObj, ((ItemGrenade)arm).type, player, null);
				EntityGrenade Arm2 = new EntityGrenade(player.worldObj, ((ItemGrenade)arm).type, player, null);
				EntityGrenade Leg = new EntityGrenade(player.worldObj, ((ItemGrenade)leg).type, player, null);
				EntityGrenade Leg2 = new EntityGrenade(player.worldObj, ((ItemGrenade)leg).type, player, null);
				EntityGrenade Head = new EntityGrenade(player.worldObj, ((ItemGrenade)head).type, player, null); 
				//Spawn the entity server side
				if(!player.worldObj.isRemote)
				{
					player.worldObj.spawnEntityInWorld(Arm);
					player.worldObj.spawnEntityInWorld(Arm2);
					player.worldObj.spawnEntityInWorld(Leg);
					player.worldObj.spawnEntityInWorld(Leg2);
					player.worldObj.spawnEntityInWorld(Head);
				}
				} //end of spawn physics corpse
				
				}
				else if (hitDamage>69)
				{
					PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 15, player.dimension, "deathOverkill", false); //69 - 123 damage play 5 second scream
					
					String goreType = "gore";//spawn physics corpse for one body part
					if (decapitated)
						goreType = "goreHead";
					if (myLeg)
						goreType = "goreLeg";
					if (disarmed)
						goreType = "goreArm";
					ItemStack grenadeStack = InfoType.getRecipeElement(goreType, 1); 
					Item corpse = grenadeStack.getItem();
					if (corpse != null && corpse instanceof ItemGrenade && !player.capabilities.isCreativeMode) //no infinite spam by shooting admins
					{
					EntityGrenade grenade = new EntityGrenade(player.worldObj, ((ItemGrenade)corpse).type, player, null);  
					//Spawn the entity server side
					if(!player.worldObj.isRemote)
					{
				...
				{
					PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 15, player.dimension, "deathOverkillTom", false); //40 to 69 damage play 2 second tom and jerry scream
