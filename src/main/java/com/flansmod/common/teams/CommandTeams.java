...
public class CommandTeams extends CommandBase
{
	...
	public void processCommand(ICommandSender sender, String[] split)
	{
		...
		*/
		...
		if(split[0].equals("movieProp"))
		{
			...
			TeamsManager.movieProp = Boolean.parseBoolean(split[1]);
			sender.addChatMessage(new ChatComponentText("Vehicle self-driving movie prop mode is now " + (TeamsManager.movieProp ? "enabled" : "disabled")));
			return;
		}
		...
		if(split[0].equals("planeRate")) //new admin controlled variable icarus system rate   9 4 23
		{
			if(split.length != 2)
			{
				sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <tickDelay>"));
				return;
			}
			TeamsManager.planeRate = Integer.parseInt(split[1]);
			if(TeamsManager.planeRate > 0)
				sender.addChatMessage(new ChatComponentText("Planes will spam everyone with their position data every " + split[1] + " tick(s)"));
			else 
			{
				TeamsManager.planeRate = 1;
				sender.addChatMessage(new ChatComponentText("It has to at least be 1 silly head"));
			}
			return;
		}
		if(split[0].equals("vehicleRate"))
		{
			if(split.length != 2)
			{
				sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <tickDelay>"));
			...
			if(TeamsManager.vehicleRate > 0)
				sender.addChatMessage(new ChatComponentText("Land/Sea vehicles will spam everyone with their position data every " + split[1] + " tick(s)"));
			else 
			{
		...
		if(split[0].equals("seatRate"))
		{
			...
			{
				sender.addChatMessage(new ChatComponentText("Seats/turrets will spam everyone with their data every " + split[1] + " tick(s)"));
			}
			else 
			{
		...
		if(split[0].equals("rviRange")) //2025 for the RVI render range
		{
			if(split.length != 2)
			{
				sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <range>"));
				return;
			}
			TeamsManager.rviRange = Integer.parseInt(split[1]);
			if(TeamsManager.rviRange >= 0)
				sender.addChatMessage(new ChatComponentText("Vehicle indicators will not render until manned vehicles are " + split[1] + " blocks away from the viewer"));
			else 
			{
				TeamsManager.planeRate = 100;  //back to default
				sender.addChatMessage(new ChatComponentText("It can't be negative dumb ass"));
			}
			return;
		}
		if(split[0].equals("rviRate")) //2025 for the RVI packet rate
		{
			if(split.length != 2)
			{
				sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <tickDelay>"));
			...
			TeamsManager.rviRate = Integer.parseInt(split[1]);
			if(TeamsManager.rviRate > 0 && TeamsManager.rviRate % 2 != 0)
				sender.addChatMessage(new ChatComponentText("Vehicle indicator packets will be spammed every " + split[1] + " ticks"));
			else if (TeamsManager.rviRate % 2 == 0)
			{
				TeamsManager.rviRate = TeamsManager.rviRate + 1;
				sender.addChatMessage(new ChatComponentText("It has to be an odd number or it fucks up. I changed it to every " + TeamsManager.rviRate + " ticks"));
			}
			else 
			{
				TeamsManager.rviRate = 5; //back to default
				sender.addChatMessage(new ChatComponentText("It has to at least be 1 dumb ass"));
			}
			return;
		}
		if(split[0].equals("shake")) //2024 testing for screenshake
		{
			if(split.length != 2)
			{
				sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <shake strength>"));
				return;
			}
			...
			if(player != null)
			PacketShakeRecoil.sendShakePacket(player.posX, player.posY, player.posZ, strength, 10, player.dimension);
			//FlansModClient.screenShaker(strength); //this version only works in singleplayer  yikes
			/*
			FlansModClient.screenShake = -strength;
			FlansModClient.lamperino = strength;
			//FlansModClient.minecraft.thePlayer.performHurtAnimation(); //baris showed me the secret to summon this
			EntityPlayer player = FlansModClient.minecraft.thePlayer;
			player.hurtTime = 10 + strength/7; //seems to be how much it shakes
			player.maxHurtTime = 10; //the smaller this is relative to hurt time, the longer and more bloodcurtlingly it shakes
			//100 hurt time vs 10 maxhurttime = long sperg. 150+ hurt time vs 10 maxhurttime = delayed chud rage. so i did strength/7 formula where 599 strength yields like 95 hurttime
			player.attackedAtYaw = 0.0F; //doesnt seem to be useful at all
			*/
			
		//	FlansModClient.screenMilkshakeBackup = -strength*2;
		//	FlansModClient.expiredMilk = strength;
		...
		if(split[0].equals("vehiclesBreakBlocks"))
		{
			...
			TeamsManager.driveablesBreakBlocks = false; //retconning this feature
			//sender.addChatMessage(new ChatComponentText("Vehicles will " + (TeamsManager.driveablesBreakBlocks ? "now" : "no longer") + " break blocks"));
			sender.addChatMessage(new ChatComponentText("I retconned this feature. No more breaking blocks!"));
			return;
		}
	...
	public void sendHelpInformation(ICommandSender sender, int page)
	{
		...
		case 4 :
		{
			...
			sender.addChatMessage(new ChatComponentText("/teams planeRate <ticks>"));
			sender.addChatMessage(new ChatComponentText("/teams vehicleRate <ticks>"));
			sender.addChatMessage(new ChatComponentText("/teams seatRate <ticks>"));
			sender.addChatMessage(new ChatComponentText("/teams rviRange <yards>"));
			sender.addChatMessage(new ChatComponentText("/teams rviRate <ticks>"));
			break;
		}
		}
	}
