...
public class Clowder {
    ...
    /** 派系仓库系统 */
    public ClowderStorehouse storehouse;
    ...
    /** 区块宣称映射：旗帜坐标ID -> ClaimedZone */
    public HashMap<String, ClaimedZone> claimedZones = new HashMap<String, ClaimedZone>();
...
clowder.pussy(world);
    ...
    public void saveClowder(int i, NBTTagCompound nbt) {
        ...
        */
        ...
        // 保存政治点数和科研点数
        nbt.setFloat(i + "_politicalPower", this.politicalPower);
        nbt.setFloat(i + "_researchPoints", this.researchPoints);
        
        // 保存仓库数据
        if(storehouse != null) {
            NBTTagCompound storehouseTag = new NBTTagCompound();
            storehouse.writeToNBT(storehouseTag);
            nbt.setTag(i + "_storehouse", storehouseTag);
        }
        
        // 保存国策树
        if(policyTree != null) {
            NBTTagCompound policyTag = new NBTTagCompound();
            policyTree.writeToNBT(policyTag);
            nbt.setTag(i + "_policyTree", policyTag);
        }
        
        // 保存决议系统
        if(decisionSystem != null) {
            NBTTagCompound decisionTag = new NBTTagCompound();
            decisionSystem.writeToNBT(decisionTag);
            nbt.setTag(i + "_decisionSystem", decisionTag);
        }
        
        // 保存科技树
        if(technologyTree != null) {
            NBTTagCompound techTag = new NBTTagCompound();
            technologyTree.writeToNBT(techTag);
            nbt.setTag(i + "_technologyTree", techTag);
        }
        
        // 保存生产系统
        if(productionSystem != null) {
            NBTTagCompound productionTag = new NBTTagCompound();
            productionSystem.writeToNBT(productionTag);
            nbt.setTag(i + "_productionSystem", productionTag);
        }
        
        // 保存区块宣称数据
        nbt.setInteger(i + "_claimedZonesCount", claimedZones.size());
        int zoneIndex = 0;
        for(java.util.Map.Entry<String, ClaimedZone> entry : claimedZones.entrySet()) {
            nbt.setString(i + "_zone_" + zoneIndex + "_key", entry.getKey());
            NBTTagCompound zoneTag = new NBTTagCompound();
            entry.getValue().writeToNBT(zoneTag);
            nbt.setTag(i + "_zone_" + zoneIndex, zoneTag);
            zoneIndex++;
        }
    }
    ...
    public static Clowder loadClowder(int i, NBTTagCompound nbt) {
        ...
        */
        ...
        // 加载政治点数和科研点数
        if(nbt.hasKey(i + "_politicalPower")) {
            c.politicalPower = nbt.getFloat(i + "_politicalPower");
        }
        if(nbt.hasKey(i + "_researchPoints")) {
            c.researchPoints = nbt.getFloat(i + "_researchPoints");
        }
        
        // 初始化新系统（使用默认意识形态）
        c.initializeSystems("neutral");
        
        // 加载仓库数据
        if(nbt.hasKey(i + "_storehouse")) {
            c.storehouse.readFromNBT(nbt.getCompoundTag(i + "_storehouse"));
        }
        
        // 加载国策树
        if(nbt.hasKey(i + "_policyTree")) {
            c.policyTree.readFromNBT(nbt.getCompoundTag(i + "_policyTree"));
        }
        
        // 加载决议系统
        if(nbt.hasKey(i + "_decisionSystem")) {
            c.decisionSystem.readFromNBT(nbt.getCompoundTag(i + "_decisionSystem"));
        }
        
        // 加载科技树
        if(nbt.hasKey(i + "_technologyTree")) {
            c.technologyTree.readFromNBT(nbt.getCompoundTag(i + "_technologyTree"));
        }
        
        // 加载生产系统
        if(nbt.hasKey(i + "_productionSystem")) {
            c.productionSystem.readFromNBT(nbt.getCompoundTag(i + "_productionSystem"));
        }
        
        // 加载区块宣称数据
        if(nbt.hasKey(i + "_claimedZonesCount")) {
            int zoneCount = nbt.getInteger(i + "_claimedZonesCount");
            for(int j = 0; j < zoneCount; j++) {
                String key = nbt.getString(i + "_zone_" + j + "_key");
                NBTTagCompound zoneTag = nbt.getCompoundTag(i + "_zone_" + j);
                ClaimedZone zone = ClaimedZone.readFromNBT(zoneTag, c);
                c.claimedZones.put(key, zone);
            }
        }
...
//                prestige *= (float) Math.pow(0.99, clowder.getPrestige());
     ...
     * 在创建派系或加载派系时调用
     */
    public void initializeSystems(String ideology) {
        if(storehouse == null) {
            storehouse = new ClowderStorehouse(this);
        }
        if(populationSystem == null) {
            populationSystem = new PopulationSystem(this);
        }
        if(policyTree == null) {
            policyTree = new PolicyTree(this, ideology);
        }
        if(decisionSystem == null) {
            decisionSystem = new DecisionSystem(this);
        }
        if(technologyTree == null) {
            technologyTree = new TechnologyTree(this);
        }
        if(productionSystem == null) {
     ...
     * 更新所有新系统（定期调用）
     */
    public void updateNewSystems(World world) {
        // 更新人口系统
        if(populationSystem != null) {
        ...
        // 更新国策研究（每天一次）
        if(policyTree != null) {
            policyTree.updateResearch();
        }
        
        // 更新科技研究（每小时一次）
        if(technologyTree != null && researchPoints > 0) {
            technologyTree.updateResearch(researchPoints);
        }
    }
    ...
    public void addPoliticalPower(float amount) {
        this.politicalPower += amount;
        if(this.politicalPower < 0) {
            this.politicalPower = 0;
        }
    }
    
    public boolean consumePoliticalPower(float amount) {
    ...
    public boolean consumeResearchPoints(float amount) {
    ...
    public void addManpower(float amount) {
        if(populationSystem != null) {
            populationSystem.addManpower(amount);
        }
    }
    
    public void addEligiblePop(float amount) {
        if(populationSystem != null) {
            populationSystem.addEligiblePopulation(amount);
        }
    }
    
    public boolean consumeManpower(float amount) {
        if(populationSystem != null) {
            return populationSystem.consumeManpower(amount);
        }
        return false;
    }
    
    public boolean consumeEligiblePop(float amount) {
        if(populationSystem != null) {
            return populationSystem.consumeEligiblePopulation(amount);
        }
        return false;
    }
     ...
     * 宣称一个区块为指定类型
     */
    public boolean claimZone(ZoneType zoneType, int flagX, int flagY, int flagZ, String zoneName, World world) {
        // 检查是否有足够的资金
        ZoneConfig config = ZoneConfig.getConfig(zoneType);
        if(config != null && money >= config.buildCost) {
            addMoney(-config.buildCost);
            
            String identifier = flagX + "_" + flagY + "_" + flagZ;
            ClaimedZone zone = new ClaimedZone(this, zoneType, flagX, flagY, flagZ, zoneName);
            claimedZones.put(identifier, zone);
     ...
     * 取消区块宣称
     */
    public boolean unclaimZone(int flagX, int flagY, int flagZ, World world) {
        String identifier = flagX + "_" + flagY + "_" + flagZ;
        if(claimedZones.containsKey(identifier)) {
            claimedZones.remove(identifier);
            ClowderData.getData(world).markDirty();
            return true;
        }
        return false;
     ...
     * 获取区块宣称信息
     */
    public ClaimedZone getClaimedZone(int flagX, int flagY, int flagZ) {
        String identifier = flagX + "_" + flagY + "_" + flagZ;
        return claimedZones.get(identifier);
    }
