            ...
            */
            ...
            case "claim":{
                if (args.length > 1)
                    ...
                    sender.addChatMessage(new ChatComponentText(ERROR + "用法: /c claim <区域类型>"));
            }break;
            ...
            case "storehouse":{
                if (args.length > 1 && args[1].equalsIgnoreCase("list"))
                    cmdStorehouseList(sender);
                else
                    sender.addChatMessage(new ChatComponentText(INFO + "仓库GUI功能开发中，使用 /c storehouse list 查看内容"));
            }break;

            case "policy":{
                if (args.length > 1) {
                    if (args[1].equalsIgnoreCase("list"))
                        cmdListPolicies(sender);
                    else if (args.length > 2 && args[1].equalsIgnoreCase("unlock"))
                        cmdUnlockPolicy(sender, args[2]);
                    else
                        sender.addChatMessage(new ChatComponentText(ERROR + "用法: /c policy list 或 /c policy unlock <id>"));
                } else {
                    sender.addChatMessage(new ChatComponentText(ERROR + "用法: /c policy <list|unlock> [id]"));
                }
            }break;
            ...
            case "decision":{
                if (args.length > 1) {
                    if (args[1].equalsIgnoreCase("list"))
                        cmdListDecisions(sender);
                    else if (args.length > 2 && args[1].equalsIgnoreCase("activate"))
                        cmdActivateDecision(sender, args[2]);
                    else
                        sender.addChatMessage(new ChatComponentText(ERROR + "用法: /c decision list 或 /c decision activate <id>"));
                } else {
                    sender.addChatMessage(new ChatComponentText(ERROR + "用法: /c decision <list|activate> [id]"));
                }
            }break;
            ...
            case "research":{
                if (args.length > 1) {
                    if (args[1].equalsIgnoreCase("list"))
                        cmdListTechnologies(sender);
                    else if (args.length > 2 && args[1].equalsIgnoreCase("start"))
                        cmdResearch(sender, args[2]);
                    else if (args[1].equalsIgnoreCase("cancel"))
                        cmdCancelResearch(sender);
                    else
                        sender.addChatMessage(new ChatComponentText(ERROR + "用法: /c research <list|start|cancel> [id]"));
                } else {
                    sender.addChatMessage(new ChatComponentText(ERROR + "用法: /c research <list|start|cancel> [id]"));
                }
            }break;
            ...
            case "factory":{
                if (args.length > 1)
                    ...
                    sender.addChatMessage(new ChatComponentText(ERROR + "用法: /c factory <military|civilian>"));
            }break;
            ...
            case "produce":{
                if (args.length > 2) {
                    try {
                        ...
                        cmdProduce(sender, args[1], quantity);
                    } catch (NumberFormatException e) {
                        sender.addChatMessage(new ChatComponentText(ERROR + "数量必须是数字！"));
                    }
                } else if (args.length > 1) {
                    cmdProduce(sender, args[1], 1);
                } else {
                    sender.addChatMessage(new ChatComponentText(ERROR + "用法: /c produce <id> [数量]"));
                }
            }break;

            case "production":{
                cmdProductionList(sender);
            }break;
...
*/
    ...
    private void cmdClaim(ICommandSender sender, String zoneTypeName) {
        ...
        if (clowder == null) {
            sender.addChatMessage(new ChatComponentText(ERROR + "你不属于任何派系！"));
            return;
        }
        
        if (clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有官员和领袖可以宣称区块！"));
            return;
        }
        
        com.hfr.clowder.ZoneType zoneType = com.hfr.clowder.ZoneType.fromDisplayName(zoneTypeName);
        if (zoneType == com.hfr.clowder.ZoneType.NONE) {
            sender.addChatMessage(new ChatComponentText(ERROR + "无效的区域类型！"));
            sender.addChatMessage(new ChatComponentText(INFO + "可用类型：居民区、行政区、矿产区、科研区、工业区"));
            return;
        }
        ...
        if (clowder.chunkManager.claimChunk(chunkX, chunkZ, zoneType, clowder)) {
            sender.addChatMessage(new ChatComponentText(INFO + "成功宣称区块(" + chunkX + "," + chunkZ + ")为" + zoneType.getDisplayName() + "！"));
            clowder.save(player.worldObj);
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "宣称失败！可能是资金不足或区块已被宣称。"));
        }
    }
    ...
    private void cmdUnclaim(ICommandSender sender) {
        ...
        if (clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有官员和领袖可以取消宣称！"));
            return;
        }
        ...
        if (clowder.chunkManager.unclaimChunk(chunkX, chunkZ)) {
            sender.addChatMessage(new ChatComponentText(INFO + "成功取消区块宣称！"));
            clowder.save(player.worldObj);
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "此区块未被你的派系宣称！"));
        }
    }
     ...
     * 显示派系状态
     */
    private void cmdStatus(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "=== " + clowder.name + " 派系状态 ==="));
        
        // 人口信息
        if (clowder.populationSystem != null) {
            sender.addChatMessage(new ChatComponentText(INFO + "总人口: " + clowder.populationSystem.getTotalPopulation()));
            sender.addChatMessage(new ChatComponentText(INFO + "人力: " + clowder.populationSystem.getManpower()));
            sender.addChatMessage(new ChatComponentText(INFO + "适役人口: " + clowder.populationSystem.getRecruitablePopulation()));
        }
        
        // 政治和经济
        sender.addChatMessage(new ChatComponentText(LIST + "政治点数: " + clowder.getPoliticalPoints()));
        sender.addChatMessage(new ChatComponentText(LIST + "金钱: " + String.format("%.0f", clowder.getMoney())));
        sender.addChatMessage(new ChatComponentText(LIST + "声望: " + String.format("%.0f", clowder.getPrestige())));
        
        // 领土统计
        if (clowder.chunkManager != null) {
            sender.addChatMessage(new ChatComponentText(TITLE + "--- 领土统计 ---"));
            sender.addChatMessage(new ChatComponentText(HELP + "居民区: " + clowder.chunkManager.getChunkCountByType(com.hfr.clowder.ZoneType.RESIDENTIAL)));
            sender.addChatMessage(new ChatComponentText(HELP + "行政区: " + clowder.chunkManager.getChunkCountByType(com.hfr.clowder.ZoneType.ADMINISTRATIVE)));
            sender.addChatMessage(new ChatComponentText(HELP + "矿产区: " + clowder.chunkManager.getChunkCountByType(com.hfr.clowder.ZoneType.MINING)));
            sender.addChatMessage(new ChatComponentText(HELP + "科研区: " + clowder.chunkManager.getChunkCountByType(com.hfr.clowder.ZoneType.RESEARCH)));
            sender.addChatMessage(new ChatComponentText(HELP + "工业区: " + clowder.chunkManager.getChunkCountByType(com.hfr.clowder.ZoneType.INDUSTRIAL)));
            sender.addChatMessage(new ChatComponentText(HELP + "军用工厂: " + clowder.chunkManager.getTotalMilitaryFactories()));
            sender.addChatMessage(new ChatComponentText(HELP + "民用工厂: " + clowder.chunkManager.getTotalCivilianFactories()));
        }
        
        // 生产能力
        if (clowder.productionSystem != null) {
            sender.addChatMessage(new ChatComponentText(TITLE + "--- 生产能力 ---"));
            sender.addChatMessage(new ChatComponentText(LIST + "总生产槽位: " + clowder.productionSystem.getTotalProductionSlots() + " (已用: " + clowder.productionSystem.getUsedProductionSlots() + ")"));
        }
        
        // 研究进度
        if (clowder.techTreeSystem != null && clowder.techTreeSystem.getResearchingTech() != null) {
            sender.addChatMessage(new ChatComponentText(TITLE + "--- 当前研究 ---"));
            sender.addChatMessage(new ChatComponentText(LIST + "科技: " + clowder.techTreeSystem.getResearchingTech() + " (" + String.format("%.1f", clowder.techTreeSystem.getResearchProgress()) + "%)"));
        }
    }
     ...
     * 查看仓库内容
     */
    private void cmdStorehouseList(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "=== 派系仓库内容 ==="));
        
        java.util.List<String> summary = clowder.storehouse.getInventorySummary();
        if (summary.isEmpty()) {
            sender.addChatMessage(new ChatComponentText(HELP + "仓库是空的"));
        } else {
            for (String item : summary) {
                sender.addChatMessage(new ChatComponentText(INFO + item));
            }
        }
    }
     ...
     * 列出所有国策
     */
    private void cmdListPolicies(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "=== 可用国策 ==="));
        
        java.util.Map<String, java.util.Map<String, Object>> policies = com.hfr.config.SimpleConfigHandler.getPolicies();
        if (policies.isEmpty()) {
            sender.addChatMessage(new ChatComponentText(HELP + "没有可用的国策"));
            return;
        }
        
        for (java.util.Map.Entry<String, java.util.Map<String, Object>> entry : policies.entrySet()) {
            String id = entry.getKey();
            java.util.Map<String, Object> policy = entry.getValue();
            boolean unlocked = clowder.policySystem != null && clowder.policySystem.isPolicyUnlocked(id);
            String name = (String) policy.getOrDefault("name", id);
            int cost = ((Number) policy.getOrDefault("cost", 0)).intValue();
            
            String status = unlocked ? EnumChatFormatting.GREEN + "[已解锁] " : EnumChatFormatting.GRAY + "[未解锁] ";
            sender.addChatMessage(new ChatComponentText(status + name + " (ID: " + id + ", 费用: " + cost + ")"));
        }
    }
     ...
     * 解锁国策
     */
    private void cmdUnlockPolicy(ICommandSender sender, String policyId) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        if (clowder.getPermLevel(player.getDisplayName()) < 3) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有领袖可以解锁国策！"));
            return;
        }
        
        if (clowder.policySystem.unlockPolicy(policyId, clowder)) {
            sender.addChatMessage(new ChatComponentText(INFO + "成功解锁国策：" + policyId));
            clowder.save(player.worldObj);
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "解锁失败！可能是政治点数不足或前置条件未满足。"));
        }
    }
     ...
     * 列出所有决议
     */
    private void cmdListDecisions(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "=== 可用决议 ==="));
        
        java.util.Map<String, java.util.Map<String, Object>> decisions = com.hfr.config.SimpleConfigHandler.getDecisions();
        if (decisions.isEmpty()) {
            sender.addChatMessage(new ChatComponentText(HELP + "没有可用的决议"));
            return;
        }
        
        for (java.util.Map.Entry<String, java.util.Map<String, Object>> entry : decisions.entrySet()) {
            String id = entry.getKey();
            java.util.Map<String, Object> decision = entry.getValue();
            boolean active = clowder.decisionSystem != null && clowder.decisionSystem.isDecisionActive(id);
            String name = (String) decision.getOrDefault("name", id);
            int cost = ((Number) decision.getOrDefault("cost", 0)).intValue();
            int duration = ((Number) decision.getOrDefault("duration", 24)).intValue();
            
            String status = active ? EnumChatFormatting.GREEN + "[生效中] " : EnumChatFormatting.GRAY + "[未激活] ";
            sender.addChatMessage(new ChatComponentText(status + name + " (ID: " + id + ", 费用: " + cost + ", 持续: " + duration + "小时)"));
        }
    }
     ...
     * 激活决议
     */
    private void cmdActivateDecision(ICommandSender sender, String decisionId) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        if (clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有官员和领袖可以激活决议！"));
            return;
        }
        
        if (clowder.decisionSystem.activateDecision(decisionId, clowder)) {
            sender.addChatMessage(new ChatComponentText(INFO + "成功激活决议：" + decisionId));
            clowder.save(player.worldObj);
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "激活失败！可能是政治点数不足或决议已在生效中。"));
        }
    }
     ...
     * 列出所有科技
     */
    private void cmdListTechnologies(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "=== 科技树 ==="));
        
        java.util.Map<String, java.util.Map<String, Object>> techs = com.hfr.config.SimpleConfigHandler.getTechnologies();
        if (techs.isEmpty()) {
            sender.addChatMessage(new ChatComponentText(HELP + "没有可用的科技"));
            return;
        }
        
        for (java.util.Map.Entry<String, java.util.Map<String, Object>> entry : techs.entrySet()) {
            String id = entry.getKey();
            java.util.Map<String, Object> tech = entry.getValue();
            boolean unlocked = clowder.techTreeSystem != null && clowder.techTreeSystem.isTechUnlocked(id);
            String name = (String) tech.getOrDefault("name", id);
            int cost = ((Number) tech.getOrDefault("cost", 0)).intValue();
            
            String status = unlocked ? EnumChatFormatting.GREEN + "[已解锁] " : EnumChatFormatting.GRAY + "[未解锁] ";
            sender.addChatMessage(new ChatComponentText(status + name + " (ID: " + id + ", 科研点: " + cost + ")"));
        }
    }
     ...
     * 开始研究科技
     */
    private void cmdResearch(ICommandSender sender, String techId) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        if (clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有官员和领袖可以开始研究！"));
            return;
        }
        
        if (clowder.techTreeSystem.startResearch(techId, clowder)) {
            sender.addChatMessage(new ChatComponentText(INFO + "开始研究科技：" + techId));
            clowder.save(player.worldObj);
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "研究失败！可能是已经在研究其他科技或前置条件未满足。"));
        }
    }
     ...
     * 取消当前研究
     */
    private void cmdCancelResearch(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        if (clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有官员和领袖可以取消研究！"));
            return;
        }
        
        clowder.techTreeSystem.cancelResearch();
        sender.addChatMessage(new ChatComponentText(INFO + "已取消当前研究"));
        clowder.save(player.worldObj);
    }
    ...
    private void cmdFactory(ICommandSender sender, String factoryType) {
        ...
        if (clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有官员和领袖可以建造工厂！"));
            return;
        }
        
        boolean military = factoryType.equalsIgnoreCase("military");
        if (!military && !factoryType.equalsIgnoreCase("civilian")) {
            sender.addChatMessage(new ChatComponentText(ERROR + "工厂类型必须是 military 或 civilian"));
            return;
        }
        ...
        if (clowder.chunkManager.buildFactory(chunkX, chunkZ, military, clowder)) {
            ...
            sender.addChatMessage(new ChatComponentText(INFO + "成功建造" + type + "工厂！"));
            clowder.save(player.worldObj);
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "建造失败！确保此区块是工业区且有足够资金。"));
        }
    }
     ...
     * 添加生产任务
     */
    private void cmdProduce(ICommandSender sender, String productionId, int quantity) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        if (clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有官员和领袖可以添加生产任务！"));
            return;
        }
        
        if (clowder.productionSystem.addProduction(productionId, quantity, clowder)) {
            sender.addChatMessage(new ChatComponentText(INFO + "成功添加生产任务：" + productionId + " x" + quantity));
            clowder.save(player.worldObj);
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "添加失败！可能是生产槽位已满、材料不足或科技未解锁。"));
        }
    }
     ...
     * 查看生产队列
     */
    private void cmdProductionList(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "=== 生产队列 ==="));
        sender.addChatMessage(new ChatComponentText(LIST + "总槽位: " + clowder.productionSystem.getTotalProductionSlots() + " (已用: " + clowder.productionSystem.getUsedProductionSlots() + ")"));
        
        java.util.List<com.hfr.clowder.ProductionSystem.ProductionTask> queue = clowder.productionSystem.getProductionQueue();
        if (queue.isEmpty()) {
            sender.addChatMessage(new ChatComponentText(HELP + "没有正在进行的生产任务"));
        } else {
            int index = 1;
            for (com.hfr.clowder.ProductionSystem.ProductionTask task : queue) {
                String progress = String.format("%.1f", task.getProgress());
                int remaining = task.getRemainingTime();
                sender.addChatMessage(new ChatComponentText(INFO + index + ". " + task.productionId + " x" + task.quantity + " (" + progress + "%, 剩余: " + remaining + "秒)"));
                index++;
            }
        }
    }
