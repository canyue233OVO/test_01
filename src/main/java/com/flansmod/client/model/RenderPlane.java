...
{	
    ...
    public void render(EntityPlane entityPlane, double d, double d1, double d2, float f, float f1)
    {
		...
		if(model != null)
		{
	        ...
	        //shrink planes on carrier
	        float division = 1f; //new for shrink plane
	        if (!type.parasitePlane) //new for shrink plane
	        division =  (float)(18.8f + entityPlane.driveableData.catapult)/18.8f; //number that goes from 1 to 3.33 if on a ship;
	        if(entityPlane.mode == EnumPlaneMode.HELI && entityPlane.mounted) //no animation but still shrink for helis
	        	division = 3.3f;
			{
			GL11.glScalef(type.modelScale/division, type.modelScale/division, type.modelScale/division); //apply the carrier shrinkage
			}
...
//missile render attempt
		        if(TeamsManager.shellsEnabled)
				{
					...
					for(int i = entityPlane.getDriveableData().getBombInventoryStart(); i < entityPlane.getDriveableData().getBombInventoryStart() + type.numBombSlots; i++)
					{
						ItemStack bomb = entityPlane.getDriveableData().getStackInSlot(i);
				 ...
				 if(item instanceof ItemBullet && ((ItemBullet)item).type.model != null && item.type.wingVisible && type.missileVisible) //shitty old version
				{
				...
				}   //end of missile render attempt  original version
				 ...
				 if(item instanceof ItemBullet && ((ItemBullet)item).type.model != null && (type.missilePrimaryAuto||type.missileSecondaryAuto)) //2024 improved version
					{
						...
						if (point!= null && point.rootPos!= null && point.rootPos.part!= null && entityPlane.isPartIntact(point.rootPos.part))
						{
						//GL11.glRotatef(1F, 1F, 0F, 0F);
						
						GL11.glTranslatef(-(lastY-y), -(lastX-x), (lastZ-z));
						lastX = x; 
						lastY = y;
						lastZ = z;
