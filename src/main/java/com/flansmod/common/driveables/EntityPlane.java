...
import com.flansmod.common.network.PacketTargetSwitch;
...
import bariss26.remastered.alet.BarisTechTreeHandler;
import bariss26.remastered.alet.BarisTechTreeHandler.BarisPacket;
...
public class EntityPlane extends EntityDriveable
{
	...
	public boolean pressKey(int key, EntityPlayer player)
	{
		...
		case 7 : //Inventory : Check to see if this plane allows in-flight inventory editing or if the plane is on the ground
		{
			...
			//if(worldObj.isRemote && this.control.V < 0.2f*type.takeoffSpeed && throttle < 0.1f) //speed based inventory restriction 2024
			{
				FlansMod.proxy.openDriveableMenu((EntityPlayer)seats[0].riddenByEntity, worldObj, this);
			}
			//else
				//player.addChatMessage(new ChatComponentText("You must slow down to access your aircraft's inventory" ));
			return true;
		}
		...
		{
			FlansMod.proxy.changeControlMode((EntityPlayer)this.seats[0].riddenByEntity); //added a this to prevent mixing with other players june 16 2023
		...
		case 14 : // Door
		{
			...
			if(toggleTimer <= 0)
			{
				...
				toggleTimer = 10;
				FlansMod.getPacketHandler().sendToServer(new PacketDriveableControl(this));
				//System.out.println("door is " + varDoor);
			}
			/*
			if(toggleTimer <= 0)
			{
		...
		{
			if( this.ticksRepairing <= 0 && throttle <= 0.05 && !worldObj.isAirBlock((int)posX, (int)(posY-5), (int)posZ) && 25843 > type.repairCost) //2025 TO DO when baris gives me power to check your money, replace 25843 with player balance
			{
				this.ticksRepairing = type.ticksRepairing;

				PacketPlaySound.sendSoundPacket(posX, posY, posZ, 10f, dimension, "Repair", false);
				
		        BarisTechTreeHandler.BarisPacket packet = new BarisPacket(); //2025 upgrade this to take away money
		        packet.add("method", "addXP");
		        packet.add("type", "money");
		        packet.add("amount", -type.repairCost);
		        packet.add("player", player.getDisplayName());
		        BarisTechTreeHandler.sendMessage(packet);
		...
		case 41 : //for target lock switch 
		{
			if(driveableData.targetSwitchTimer <= 0 && (type.lockOnToPlanes || type.lockOnToVehicles || type.lockOnToMechas || type.lockOnToLivings || type.lockOnToPlayers))
			{
				int id1 =25843;
				int id2 =25843;
				int idMe =25843;
				if(LockedTarget!=null)
					id1=LockedTarget.getEntityId();
				if(ToggleLocked!=null)
					id2=ToggleLocked.getEntityId();
				if(((EntityDriveable)this)!=null)
					idMe = ((EntityDriveable)this).getEntityId();
				FlansMod.getPacketHandler().sendToServer(new PacketTargetSwitch(id1, id2, idMe)); //this guy should do all the work
				//if(seats != null && seats[0].riddenByEntity != null && seats[0].riddenByEntity instanceof EntityPlayerMP)
				// FlansMod.getPacketHandler().sendTo(new PacketTargetSwitch(id1, id2, idMe), (EntityPlayerMP)seats[0].riddenByEntity); //client version  failure

				driveableData.targetSwitchTimer = 10;
			}
			return true;
		}
	...
	public void onUpdate()
	{
		...
		//2025 open missile doors automatically FAILURE
		if(type!= null && leftMouseHeld && type.primaryDoor)
		{
			varDoor = true;
			System.out.println("primary door");
		}
		
		if(type!= null && rightMouseHeld && type.secondaryDoor)
		{
			varDoor = true;
			System.out.println("secondary door");
		}
		...
		if(this.ticksExisted > 20)
		{
			...
			//master is online check  retconned 9 4 23
			/* if (getDriveableData().seatBelt != null && !getDriveableData().seatBelt.equals("null") && getDriveableData().seatBelt.length() != 0)
			{
				...
				if(worldObj.getPlayerEntityByName(getDriveableData().seatBelt) == null) //check if master offline
				{
					...
					{
						EntityPlayer Master = worldObj.getPlayerEntityByName(getDriveableData().seatBelt);
					}
		...
		if(TeamsManager.movieProp)
			varGear = false; //turn off gear for self flying prop planes
		...
		//2024 target switch
		if(driveableData.targetSwitchTimer > 0)
			driveableData.targetSwitchTimer--;
		...
		*/
		...
		if(mounted)
		{
			if(mode != EnumPlaneMode.HELI && driveableData.catapult < 44) //no cataboost for heli  also new: make it gradually go up so i can see funny shrinking animation
				driveableData.catapult += 2;
			varGear = true;
		...
		for(EntitySeat seat : seats)
		{
			...
			if( (seat != null && seat.riddenByEntity != null && this.ticksExisted % TeamsManager.seatRate == 0) || (seat != null && seat.riddenByEntity == null && this.ticksExisted % 30 == 0) ) 
				seat.updatePosition();  //case 1: teams manager variable delay rate     case 2: empty seat  20x slow
		}
		...
		if(thePlayerIsDrivingThis && this.ticksExisted % TeamsManager.planeRate == 0) //9 4 2023 improved icarus system
		{
		...
		if(!worldObj.isRemote && this.ticksExisted % TeamsManager.planeRate == 0)// && (Math.abs(posX - prevPosX) > updateSpeed || Math.abs(posY - prevPosY) > updateSpeed || Math.abs(posZ - prevPosZ) > updateSpeed))
		{
