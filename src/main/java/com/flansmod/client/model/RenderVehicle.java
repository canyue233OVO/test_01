...
public class RenderVehicle extends Render implements IItemRenderer
{
    ...
    public void render(EntityVehicle vehicle, double d, double d1, double d2, float f, float f1)
    {
        ...
        GL11.glPushMatrix();
        {
			...
			GL11.glPushMatrix();
			{
				ModelVehicle modVehicle = null;
				//ModelVehicle modVehicle = (ModelVehicle)type.modelShitty;
				
				//if(vehicle.throttle > 0.5f) //2025 levelofdetail experiment
				//	modVehicle = (ModelVehicle)type.modelShitty;
				//else
				//	modVehicle = (ModelVehicle)type.model;
				...
				GL11.glPushMatrix();
				{
					...
					float distance = vehicle.getDistanceToEntity(Minecraft.getMinecraft().thePlayer);
					
					modVehicle = (ModelVehicle)type.modelShitty;
					
					if(distance > 40) //2025 levelofdetail experiment 40 range
						modVehicle = (ModelVehicle)type.modelShitty;
					else
						modVehicle = (ModelVehicle)type.model;
				...
				if(vehicle.Evolving)//color change while evolving pokemon 2024
				{
					float sneed = (float)Math.sin(vehicle.dynamicScale);
				GL11.glColor4f(1F, sneed*1F, sneed*1F, sneed*1F);
				}
				
				//for size animations 2024
		        float scaleMultiplier = (float)vehicle.dynamicScale/100;     
					GL11.glScalef(type.modelScale*scaleMultiplier, type.modelScale*scaleMultiplier, type.modelScale*scaleMultiplier);
					if(modVehicle != null)
				...
				//missile render attempt  //ported from plane 2024
		        if(TeamsManager.shellsEnabled)
				{
					int slot = -1;
					for(int i = vehicle.getDriveableData().getMissileInventoryStart(); i < vehicle.getDriveableData().getMissileInventoryStart() + type.numMissileSlots; i++)
					...
					//again for bombs 2024
					for(int i = vehicle.getDriveableData().getBombInventoryStart(); i < vehicle.getDriveableData().getBombInventoryStart() + type.numBombSlots; i++)
					{
						ItemStack bomb = vehicle.getDriveableData().getStackInSlot(i);
				 ...
				 if(item instanceof ItemBullet && ((ItemBullet)item).type.model != null && (type.missilePrimaryAuto||type.missileSecondaryAuto)) //2024 improved version
					{
						BulletType gunType = ((ItemBullet)item).type;
						ModelBase modelo = item.type.model;
						...
						if (item.type.hasLauncherModel)  //smarter version of rocket pod model option render. just overwrite model at beginning instead of duplicating the renderer
						{
							modelo = item.type.launcherMesh;
							texturemanager.bindTexture(FlansModResourceHandler.getAlternateTexture(gunType));
						}
						

							texturemanager.bindTexture(FlansModResourceHandler.getTexture(gunType));
							//GL11.glRotatef(90F, 0F, 1F, -90F); //it was the first and last 1 that were -90
						ItemRenderType typerino = ItemRenderType.ENTITY;
						
						float x = 0;
						float y = 0;
						float z = 0;
						float lastX = x; //you have to fucking translate the model relative to previous position instead of simply setting coord so i need temporary memory of previous position
						float lastY = y;
						float lastZ = z;
						
						ArrayList thisone = null;
						if (type.missilePrimaryAuto)
							thisone = type.shootPointsPrimary;
						else if(type.missileSecondaryAuto)
							thisone = type.shootPointsSecondary;
						
						if(thisone!=null && (thisone).size() > 0)
						{
							GL11.glRotatef(90F, 0F, 1F, 0F);	
							GL11.glRotatef(90F, 1F, 0F, 0F);
							
							// X Y Z    +X = negative Z (to left wing direction)   +Y = positive X forward   +Z = negative Y into the ground
								for (int i = 0; i < (thisone).size(); i++) 
							{
									ShootPoint point = (ShootPoint)(thisone.get(i));
								x = (point.rootPos.position.x);
								y = (point.rootPos.position.y);
								z = (point.rootPos.position.z);								
								 //System.out.println("x: " + point.rootPos.position.x);
								 //System.out.println("y: " + y);
								 //System.out.println("z: " + z);
				
						if (point!= null && point.rootPos!= null && point.rootPos.part!= null && vehicle.isPartIntact(point.rootPos.part))
						{
						//GL11.glRotatef(1F, 1F, 0F, 0F);
						
						
						
						GL11.glTranslatef((lastZ-z), -(lastX-x), (lastY-y));
						lastX = x; 
						lastY = y;
						lastZ = z;
						// System.out.println("last x: " + lastX);
						//GL11.glTranslatef(0F, 0F, 0F);
						// (a b c) positive a is towards pilot's left, positive b is up towards sky, negative c is back to front of plane	   but only if GL11.glRotatef(90F, 1F, 0F, 0F);
						//but from shootpos  X20FrontBack, Y1UpDown, Z-70LeftRight
						modelo.render(null, 0F, 0F, 0F, 0F, 0F, 1F / 16F);
						}				
							}
							}		
					} //end of 2024 imprved version again
				 }		
				} //ported from plane 2024
