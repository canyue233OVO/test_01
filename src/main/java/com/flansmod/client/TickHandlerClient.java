...
public class TickHandlerClient
{
    ...
    public static int cringeAddedWeight = 0;
    
    public static int killsTotal = 0; //june 23 2023 kill count
    ...
    public void eventHandler(RenderGameOverlayEvent event)
    {
        ...
        if(!event.isCancelable() && event.type == ElementType.HELMET)
        {
            ...
            //    if(killsTotal > 0 && killsTotal < 9) //multikill system june 23 2023
            //        overlayTexture = "multikill" + killsTotal;
            //    else if(killsTotal > 9) 
            //        overlayTexture = "multikill10"; //once you reach 10, stay at nuke icon
        ...
        if(!event.isCancelable() && event.type == ElementType.HOTBAR && FlansMod.bulletGuiEnable)
        {
            ...
            if(mc.thePlayer != null)
            {
                ...
                if(cringeBlood<100)
                    mc.fontRenderer.drawString("Blood: " + cringeBlood  , 142, 30, 0xdaa520);    
                if(cringeHemorrhaging>0)
                    mc.fontRenderer.drawString("FATAL BEEDING"  , 142, 42, 0xff0000);
            ...
            if(mc.thePlayer.ridingEntity instanceof EntitySeat)
            {
                ...
                if(ent instanceof EntityPlane)
                {
                    ...
                    if(Planerino.control != null)
                    {
                        ...
                        //overload
                        if(Planerino.control.Gfactor > Planerino.getPlaneType().maxG*0.66f && Planerino.control.Gfactor < Planerino.getPlaneType().maxG)  //starts being inefficient if 2/3 to max G
                            mc.fontRenderer.drawString(String.format("OVERLOAD %.0f", Planerino.control.Gfactor) + "G", 198, 72, 0xB53333);
                        else if(Planerino.control.Gfactor >= Planerino.getPlaneType().maxG ) //starts doing damage
                ...
                if(!(ent instanceof EntityMecha) && !ent.aiming)
                {
                    ...
                    else if((ent instanceof EntityPlane) )
                    {
                        ...
                        if (Plane.driveableData.carrierTip>0 && !ent.mounted)
                            mc.fontRenderer.drawString("按 "+ Keyboard.getKeyName(KeyInputHandler.bombKey.getKeyCode())+ " 对接", 142, 62, 0x00ff00);
                    }
                }
                ...
                if(!(ent instanceof EntityPlane))
                {
                    ...
                    {
                        if(ent.getDriveableType().walterGunRange || ent.getDriveableType().walterMortar || ent.getDriveableType().gunRange) //polati millibars larper angle for artillery    june 16 2023
                        {
                            mc.fontRenderer.drawString(String.format("Gun Pitch (MIL): %.0f", ((EntitySeat)mc.thePlayer.ridingEntity).looking.getPitch() * (-1f) * 3141.59265358979323846 / 180), 142, 12, 0xffffff);
                            mc.fontRenderer.drawString(String.format("(DEG): %.2f", ((EntitySeat)mc.thePlayer.ridingEntity).looking.getPitch() * (-1f))+ "°", 300, 12, 0xffffff);
                            mc.fontRenderer.drawString(String.format("Gun Yaw (MIL): %.0f", ((EntitySeat)mc.thePlayer.ridingEntity).looking.getYaw() * 3141.59265358979323846 / 180), 142, 2, 0xffffff);
                            mc.fontRenderer.drawString(String.format("(DEG): %.2f", ((EntitySeat)mc.thePlayer.ridingEntity).looking.getYaw())+ "°", 300, 2, 0xffffff);
                        }
                        else
                        {
                        mc.fontRenderer.drawString(String.format("Gun Pitch : %.0f%%", ((EntitySeat)mc.thePlayer.ridingEntity).looking.getPitch() * (-1f)), 142, 12, 0xffffff);
                        mc.fontRenderer.drawString(String.format("Yaw : %.0f%%", ((EntitySeat)mc.thePlayer.ridingEntity).looking.getYaw()), 142, 2, 0xffffff);
                        }
                    }
                ...
                if(pitch>=70f && pitch<90)
                    displayRange = mortarRange;
                        ...
                        if (pitch > 0)
                            mc.fontRenderer.drawString(String.format("Estimated Gun Range : %.0f",  displayRange), 142, 62, 0xffffff);
                        if (displayRange > 350)
                            mc.fontRenderer.drawString(String.format("Warning: Bullet May Freeze Outside of Render Range"), 142, 72, 0xff0000);
                        ...
                        if( pitch<15f)
                            mc.fontRenderer.drawString(String.format("Warning: Predicion Less Accurate at Low Angles"), 142, 82, 0xdaa520);
                ...
                if(ent instanceof EntityPlane)
                {
                    ...
                    mc.fontRenderer.drawString(String.format("Plane Pitch : %.1f",  -1f*(float)entP.axes.getPitch()) + "°", 142, 22, 0xffffff);
                    mc.fontRenderer.drawString(String.format("Plane Roll : %.1f", + -1f*(float)entP.axes.getRoll()) + "°", 142, 12, 0xffffff);
                    if ((float)entP.axes.getYaw() < 22.5 && (float)entP.axes.getYaw() >= -22.5)
                        mc.fontRenderer.drawString("Compass : East"  , 142, 2, 0xffffff);

                    if ((float)entP.axes.getYaw() < 67.5 && (float)entP.axes.getYaw() >= 22.5)
                        mc.fontRenderer.drawString("Compass: South-East"  , 142, 2, 0xffffff);

                    if ((float)entP.axes.getYaw() >= 67.5 && (float)entP.axes.getYaw() < 112.5)
                        mc.fontRenderer.drawString("Compass : South"  , 142, 2, 0xffffff);

                    if ((float)entP.axes.getYaw() >= 112.5 && (float)entP.axes.getYaw() < 157.5)
                        mc.fontRenderer.drawString("Compass : South-West"  , 142, 2, 0xffffff);

                    if ((float)entP.axes.getYaw() < -157.5 || (float)entP.axes.getYaw() >= 157.5)
                        mc.fontRenderer.drawString("Compass : West"  , 142, 2, 0xffffff);

                    if ((float)entP.axes.getYaw() < -112.5 && (float)entP.axes.getYaw() >= -157.5)
                        mc.fontRenderer.drawString("Compass: North-West"  , 142, 2, 0xffffff);

                    if ((float)entP.axes.getYaw() > -112.5 && (float)entP.axes.getYaw() <= -67.5)
                        mc.fontRenderer.drawString("Compass : North"  , 142, 2, 0xffffff);

                    if ((float)entP.axes.getYaw() > -67.5 && (float)entP.axes.getYaw() <= -22.5)
                        mc.fontRenderer.drawString("Compass : North-East"  , 142, 2, 0xffffff);
                    ...
                    //mc.fontRenderer.drawString(String.format("Altitude : %.0f" , ((EntitySeat)mc.thePlayer.ridingEntity).posY) + " Meters", 142, 32, 0xffffff);
                    mc.fontRenderer.drawString(String.format("Altitude : %.0f" , ((EntitySeat)mc.thePlayer.ridingEntity).posY) + "m / " + entPl.getPlaneType().ceiling + "m", 142, 32, 0xffffff);

                    if (entP.getDriveableType().rocketThrottle)
                        mc.fontRenderer.drawString("Fuel : "+   entP.fuelTimer, 142, 42, 0xffffff);
                    ...
                    if (entP.mounted)
                        mc.fontRenderer.drawString("按 "+ Keyboard.getKeyName(KeyInputHandler.gunKey.getKeyCode())+ " 解除对接", 142, 62, 0xff0000);
                    ...
                    if( !entP.getDriveableType().epicShip)
                    {
                        if(entP.ticksRepairing <= 0 && entP.flareDelay <= 0 && ent.throttle < 0.01 && ent.throttle > -0.01   && !entP.worldObj.isAirBlock((int)entP.posX, (int)(entP.posY-5), (int)entP.posZ))
                            mc.fontRenderer.drawString("修理就绪 - 按 " + Keyboard.getKeyName(KeyInputHandler.repairKey.getKeyCode())  , 288, 12, 0x00ff00);
                    ...
                    {
                        mc.fontRenderer.drawString("Current Ammo ", 288, 52, 0xffffff); //display name of loaded ammo
                        mc.fontRenderer.drawString(entP.currentAmmo, 288, 62, 0x24FF62);
                    }
                    ...
                    if (entP.getDriveableType().hasPlaneRadar )
                        mc.fontRenderer.drawString("按 " + Keyboard.getKeyName(KeyInputHandler.activeRadar.getKeyCode()) + " 切换", 288, 32, 0xFF9024);
                    
                    if (entP.currentAmmo != null && entP.getDriveableData().addedWeight > 0 ) //2024 ammo weight system
                    {
                        mc.fontRenderer.drawString("Munitions Weight", 142, 52, 0xffffff);
                        int color = 0x24FF62;
                        if (entP.getDriveableData().addedWeight > 0.55f*((EntityPlane)entP).getPlaneType().mass)
                            color = 0xdaa520;
                        if (entP.getDriveableData().addedWeight > 1.25f*((EntityPlane)entP).getPlaneType().mass)
                            color = 0xff0000;
                        mc.fontRenderer.drawString(entP.getDriveableData().addedWeight + " kg", 142, 62, color);
                    }
                ...
                if(ent instanceof EntityVehicle)
                {
                    ...
                    if(((EntityVehicle) entP).getVehicleType().canRepair && !entP.getDriveableType().epicShip)
                    {
                        if(entP.ticksRepairing <= 0 && entP.flareDelay <= 0 && ent.throttle < 0.01 && ent.throttle > -0.01)
                            mc.fontRenderer.drawString("修理就绪 - 按 " + Keyboard.getKeyName(KeyInputHandler.repairKey.getKeyCode())  , 2, 42, 0x00ff00);
                    ...
                    if (entP.getDriveableType().hasRadar )
                        mc.fontRenderer.drawString("按 " + Keyboard.getKeyName(KeyInputHandler.activeRadar.getKeyCode()) + " 切换", 292, 12, 0xFF9024);
                    ...
                    if (entP.getDriveableType().shootDelayPrimary > 30)
                    {
                        ...
                        {
                            mc.fontRenderer.drawString(String.format("Reload Time: %.1f", ((float)entP.recoilTimer/20f)) + " seconds", 142, 22, 0xff0000);
                        }
                        else if(entP.recoilTimer <= 0)
                        {
                            mc.fontRenderer.drawString("Shell : Ready", 142, 22, 0x00ff00);
                        }
                    }
                    //end of recoil world
                    ...
                    {
                        mc.fontRenderer.drawString("Current Ammo ", 142, 32, 0xffffff); //display name of loaded ammo
                        mc.fontRenderer.drawString(entP.currentAmmo, 142, 42, 0x24FF62);
                    }
                    ...
                    if(((EntityVehicle)ent).getVehicleType().airship)
                        mc.fontRenderer.drawString(String.format("Diving Throttle : %.0f%%", ent.divingFactor*100), 142, 32, 0xffffff);
                    ...
                    if(((EntityVehicle)ent).getVehicleType().canDive)
                    {
                        ...
                        mc.fontRenderer.drawString(String.format("Diving Throttle : %.0f%%", ent.divingFactor*100), 142, 52, 0xffffff);

                        if(((EntityVehicle)entP).getVehicleType().unlimitedOxygen==false)
                            mc.fontRenderer.drawString("Oxygen (Seconds) : " + ent.oxygenMeter/160f  , 142, 62, colour);
                        if(((EntityVehicle)entP).getVehicleType().unlimitedOxygen==true)
                            mc.fontRenderer.drawString("Unlimited Oxygen", 142, 62, colour);
                        ...
                        if(poopooDepth>0)
                        {
                            ...
                            if(poopooDepth / (float)allah.maxDepth > 0.7)
                                mc.fontRenderer.drawString("WARNING : Aproaching Max Depth"  , 142, 82, 0xdaa520);

                            if(poopooDepth / (float)allah.maxDepth > 0.9)
                                mc.fontRenderer.drawString("DANGER : COLLAPSE IMMINENT"  , 142, 92, 0xff0000);
                        }
