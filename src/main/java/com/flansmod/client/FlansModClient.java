...
public class FlansModClient extends FlansMod
{
	...
	public static float sustainedRecoilYaw;
			...
			//2024 make a universal void for the screenshake instead of command and retyping it
			public static void screenShaker(int strength)
			{
				if(strength > 600)
					strength = 500;
			...
			public static void tick() {
				...
				if(screenShake < -600) //at around -650, the zoom is so strong that the screen flips upside down. dont want that!
					screenShake = -500;
				if (screenShake > 0)
					screenShake = 0;
				if (screenShake < -140)
					screenShake += 20;
				else  if (screenShake < -130)
					screenShake += 12;
				else  if (screenShake < -120)
					screenShake += 10;
				else  if (screenShake < -110)
					screenShake += 8;
				else  if (screenShake < -100)
					screenShake += 6.5;
          ...
          ObfuscationReflectionHelper.setPrivateValue(EntityRenderer.class, Minecraft.getMinecraft().entityRenderer, 1, "cameraZoom");
			...
			public static EntityFX getParticle(final String s, final World w, final double x, final double y, final double z) {
				...
				else if (s.equals("NuFlash")) {
					fx = (EntityFX)new EntityNuFlash(w, x, y, z, 0.0, 0.0, 0.0);
				}
				else if (s.equals("NuMuzzle")) {
					fx = (EntityFX)new EntityNuMuzzle(w, x, y, z, 0.0, 0.0, 0.0);
				}
				else if (s.equals("NuSpark")) {
					fx = (EntityFX)new EntityNuSpark(w, x, y, z, 0.0, 0.0, 0.0);
				}
				else if (s.equals("TurretTest")) {
					fx = (EntityFX)new EntityTurretTest(w, x, y, z, 0.0, 0.0, 0.0);
				}
