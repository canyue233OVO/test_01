                    ...
                    for (EntityPlayer entity : entities) {
                        ...
                        if (player.worldObj.getHeightValue((int) player.posX, (int) player.posZ) <= player.posY + 2
                            ...
                            //new boolean LABJAC ALLAH BOOKMARK : now enemy blips appear normally if active radar OR blips appear if enemy active radar and you passive radar
                                ...
                                Vec3 veco = Vec3.createVectorHelper(player.posX - entBogey.posX, player.posY - entBogey.posY, player.posZ - entBogey.posZ);  //find the distance 2025
                                veco.rotateAroundY((float) Math.toRadians(1));                                
                                float dist = (float) vec.lengthVector();
                                
                                blips.add(new Blip((float) -vec.xCoord, (float) vec.yCoord, (float) -vec.zCoord,
                                        (float) entBogey.posX, (float) entBogey.posZ, type, dist));
                        ...
                        if (!missile.entityMissileRadarVisible)
                            continue;
                                ...
                                blips.add(new Blip((float) -veco.xCoord, (float) veco.yCoord, (float) -veco.zCoord,
                                        (float) missile.posX, (float) missile.posZ, type, 0));
                    }
...
public void handleRVITick(TickEvent.PlayerTickEvent event) {
    ...
    if(!player.worldObj.isRemote && event.phase == Phase.START) {
        ...
        int range = 1000;    //the maximum distance where vehicles are visible
        int buffer = TeamsManager.rviRange;    //the minimum distance where vehicles are visible, can be changed with flan command 2025. Important since we never know exactly how far until real plane entity derenders per server so you can test ingame yourself
        int delay = TeamsManager.rviRate;        //the time in ticks between scans, can be changed with flan command 2025
        ...
        for(EntityPlayer entity : entities) {
            ...
            if(player.worldObj.getHeightValue((int)player.posX, (int)player.posZ) <= player.posY + 2 &&
                ...
                if (bogey !=null && bogey instanceof EntityDriveable)
                {
                    EntityDriveable Bandit = (EntityDriveable)bogey;
                    
                    if(Bandit instanceof EntityPlane)
                    {
                    mx = (float) Bandit.motionX;
                    my = (float) Bandit.motionY;
                    mz = (float) Bandit.motionZ;
                    }
                    
                    else if(Bandit instanceof EntityVehicle && ((EntityVehicle)Bandit).wheels[1]!=null)
                    {
                    mx = (float) ((EntityVehicle)Bandit).wheels[1].motionX;
                    my = (float) ((EntityVehicle)Bandit).wheels[1].motionY;
                    mz = (float) ((EntityVehicle)Bandit).wheels[1].motionZ;
                    }
                    

                    rotation = 90 - Bandit.axes.getYaw();
                    if(bogey instanceof EntityPlane)
                    {
                        rotation = 270 - Bandit.axes.getYaw(); //planes are 180 degrees off from tanks
                        pitch = Bandit.axes.getPitch();
                        roll = Bandit.axes.getRoll();
                    }
                    shortName = Bandit.getDriveableType().shortName;
                }
                
                //indicators.add(new Indicator(entBogey.posX, entBogey.posY + 2, entBogey.posZ, type));
                indicators.add(new Indicator(shortName, type, entBogey.posX, entBogey.posY, entBogey.posZ, rotation, pitch, roll, mx, my, mz, System.currentTimeMillis())); //, ry, rz, mx, my, mz); only needed the yaw rotation    add stuff to the Bogey.posY to offset the hologram up and down
            }
        }
    ...
    public void onWorldTick(WorldTickEvent event) {
        ...
        if(!world.isRemote && event.phase == Phase.START) {
            ...
            // 更新所有Clowder的系统（每小时一次 = 72000 ticks）
            if(timer % (72000) == 0) {
                for(Clowder clowder : Clowder.clowders) {
                    // 更新区域产出
                    clowder.updateAllZones(1.0f); // 1小时
                    
                    // 更新人口系统
                    clowder.populationSystem.updatePopulation();
                    clowder.populationSystem.updateProduction();
            ...
            // 更新科技研究（每10分钟 = 12000 ticks）
            if(timer % (12000) == 0) {
                for(Clowder clowder : Clowder.clowders) {
                    clowder.technologyTree.updateResearch(clowder.getResearchPoints());
                }
            }
