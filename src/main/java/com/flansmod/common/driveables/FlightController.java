...
public class FlightController {
    ...
    public void fly(EntityPlane plane) {
       ...
       // this.thrust = 0.01f * (type.maxThrottle + (data.engine == null ? 0.0f : data.engine.engineSpeed));
        ...
        float EffectiveMass = type.mass + plane.getDriveableData().addedWeight; //2024 dynamic weight
        
        //convert takeoff speed to m*tric system
        this.stallMS = 0.44704f*type.takeoffSpeed;
  
        //lift coefficient
        this.Lc = (float) ((9.8*type.mass)/(stallMS*stallMS)); //dont use effective mass here, this part is simulating the plane's design characteristics
        
        //corrective lift coefficient
        this.Lcc = (float) ((Lc*(0.44704f*type.maxSpeed*0.44704f*type.maxSpeed) - type.mass * 9.8) / (type.climbRate*type.climbRate));  //dont use effective mass here, this part is simulating the plane's design characteristics
    ...
    public void SetAxes(EntityPlane plane) {
    	float effectiveMass2 = plane.getPlaneType().mass + plane.getDriveableData().addedWeight; //2024 dynamic weight
      ...
      //  float f = sensitivityAdjust = Math.abs((float)plane.getSpeedXYZ() / 2.0f > 0.5f ? 1.5f - (float)plane.getSpeedXYZ() / 2.0f : 4.0f * (float)plane.getSpeedXYZ() / 2.0f - 1.0f);
        ...
        float f3 = sensitivityAdjust = ((this.V*2.23694f - nuTakeoffMPH)/3f) / ((type.maxSpeed - nuTakeoffMPH)/2f) > 0.2f ? 0.2f : ((this.V*2.23694f - nuTakeoffMPH)/3f) / ((type.maxSpeed - nuTakeoffMPH)/2f);  //2024 replace takeoffspeed with dynamic version
       // float f = sensitivityAdjust = Math.abs((float)labjacTickSpeed / 2.0f > 0.5f ? 1.5f - (float)labjacTickSpeed / 2.0f : 4.0f * (float)labjacTickSpeed / 2.0f - 1.0f);
        if(this.V * 2.23694f < nuTakeoffMPH) //you cant takeoff if too heavy
        	sensitivityAdjust = 0;
       ...
       float wingLoad = effectiveMass2/plane.getPlaneType().area;  //your maneuvrability is going to get FUCKED hard by weight   2024
       ...
       // System.out.println("elite agility from 0 to 3 :" + agilityE);
        ...
        float power = plane.getPlaneType().maxSpeed * plane.getPlaneType().maxSpeed / effectiveMass2; //bastardized power to weight equivalent from 3 for heavy bombers to 200 for f-15   2024 gets fucked by ammo weight
      ...
      //  float penalty = 6 - 3.5f*(power/200); //high power leads to only divided by 2.5, horrible power means divided by 6
        ...
        if (Math.abs(this.yawControl) > 0.1f && this.V > 0)
        {
        	...
        	{
        	 this.V = (this.V/(1 + ( 7*((speedBleedGrace-power/4)/(power*0.75f)) ) *this.V*this.V/(0.7f*effectiveMass2*effectiveMass2))); //speed bleed
         ...
         if (speedBleedGrace > 0)
        {
         ...
         if(Gfactor > type.maxG*0.5f) //2025 upgraded to adjust to configged maximum g factor
         {
        	 agility *= 0.99f;
        	 lowpowerAgility *= 0.99f;
         }
         
         if(Gfactor > type.maxG*0.6f)
         {
        	 agility *= 0.98f;
         ...
         if(Gfactor > type.maxG*0.7f)
         {
        	 agility *= 0.9f;
        	 lowpowerAgility *= 0.9f;
         }
         
         if(Gfactor > type.maxG && plane.isPartIntact(EnumDriveablePart.airframe))
         {
        	 agility *= 0.8f;
        	 lowpowerAgility *= 0.8f;
        	 
         		float hullDamage = 1 + Gfactor * 0.001f*(plane.getDriveableData().parts.get(EnumDriveablePart.airframe).maxHealth);
         ...
         this.Gfactor = this.V*1.2f*turnRate/18 + 1; //the x1.5 is to account for dill nerfs 2025 actually make it less sensitive, changed 1.5 to 1.2
   ...
   //     float fuelConsumptionMultiplier = 2.0f;
        ...
        float effectiveMass3 = plane.getPlaneType().mass + plane.getDriveableData().addedWeight; //2024 dynamic weight
        ...
        {
        	float mcSpeed = (float) (1.0f * Math.exp(0.487708*((this.V * 2.23694f)/767))); //convert velocity to lasttickspeed   labjac jet formula
        ...
        this.acceleration += plane.throttle*74f/10f*effectiveMass3 - this.acceleration*74f/10f*effectiveMass3;
      ...
      //  System.out.println("lasttick: " + lastTickSpeed);
        ...
        this.lift = (1 - Math.abs(forwards.y))*(this.Lc*(trueDrag) - Lcc*(this.Vh*this.Vh)) - EffectiveMass*9.8f;  //also the lift only generates if angle of attack is near horizontal. diving/climbing reduces it but gravity ignores your angle
        else  if(this.Vh > 0 && ( (!leftWing && rightWing) || (leftWing && !rightWing) )) //condition if only ONE wing is gone
        this.lift = (1 - Math.abs(forwards.y))*(this.Lc*(trueDrag)/2 - Lcc*(this.Vh*this.Vh)) - EffectiveMass*9.8f;  
        else  if(this.Vh > 0 && !leftWing && !rightWing ) //condition if both wings gone
            this.lift = (1 - Math.abs(forwards.y))*( -1 * Lcc*(this.Vh*this.Vh)) - EffectiveMass*9.8f;       
        else
       	 this.lift = (Lcc*(this.Vh*this.Vh))  - EffectiveMass*9.8f; //anti overflow when falling
       	 */
        
        //lift attempt 2
       // this.lift =  ((1 - Math.abs(forwards.y))*(this.V*this.V) - this.stallMS*this.stallMS) /( this.stallMS*this.stallMS )  ;   old formula where plane never gained weight
        
        //2024 lift speed increases the fatter the plane is
        float WeightMultiplier = (float)(effectiveMass3/type.mass);  //ratio of how fat plane is with ammo vs empty weight
        
        float nuTakeoffSpeed = (float) Math.sqrt(WeightMultiplier*this.stallMS*this.stallMS); //crude formula to find new effective takeoff speed after plane gained weight
        nuTakeoffMPH = nuTakeoffSpeed/0.44704f;
        
        this.lift =  ((1 - Math.abs(forwards.y))*(this.V*this.V) - nuTakeoffSpeed*nuTakeoffSpeed) /( nuTakeoffSpeed*nuTakeoffSpeed )  ; //2024 now takeoff speed is dynamic
        //System.out.println("takeoff speed " + nuTakeoffMPH);
        ...
        if(this.lift < -150)
        	this.lift *= 0.95f; //adjusted max fall speed/aka negative lift so you can fall a bit faster while gliding
        ...
        this.Vh += (float)this.lift/(float)(EffectiveMass);
     ...
     //System.out.println("vh " + this.Vh);
       ...
       // if(!plane.worldObj.isAirBlock((int)plane.posX, (int)(plane.posY-1), (int)plane.posZ) && throttle <= 0.2 && plane.landBrake > 5)
        ...
        { 
        	this.V += (this.T - plane.planeRecoil)/(effectiveMass3) - forwards.y*9.8f/20f + catapultBonus/(20);
        	//System.out.println("delta v recoil " + ((this.T - 20*plane.planeRecoil)/(EffectiveMass*20) - forwards.y*9.8f/20f));
       // System.out.println("V " + this.V);
        	plane.planeRecoil = 0;
        }
        else
        
        this.V += (1 + bonusFactor)*(1 + bonusFactor)*this.T/(effectiveMass3*20) - type.gravityMultiplier*forwards.y*9.8f/20f; //normal formula for velocity from accel, important!
        ...
        */
        ...
        if(Vmph <= 76 && Vmph >= 0)
        	this.dragerino = (float) (5.255f - 0.088*Vmph + 0.0005*(Vmph*Vmph));	//low speed polynomial jet formula  labjac speed formula
