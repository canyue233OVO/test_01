...
public class EntityNuSpark extends EntityFX
{
	public static ResourceLocation icon = new ResourceLocation("flansmod", "particle/NuSparkle.png");
	public EntityNuSpark(World w, double px, double py, double pz, double mx, double my, double mz)
	{
		super(w, px, py, pz, mx, my, mz);
		this.particleMaxAge = 7;
		this.particleAge = 0;
		this.particleGravity = 1;
		this.motionX = 10*mx*Math.random(); //dont want a lot of movement
		this.motionY = 10*my*Math.random();
		this.motionZ = 10*mz*Math.random();
		icon = new ResourceLocation("flansmod", "particle/NuSparkle.png");
	}
    ...
    public void renderParticle(Tessellator par1Tessellator, float par2, float par3, float par4, float par5, float par6, float par7)
    {
    	...
    	FMLClientHandler.instance().getClient().renderEngine.bindTexture(new ResourceLocation("flansmod", "particle/NuSparkle.png"));

        float scale = 0.25F - ((this.particleAge)*0.02F);
