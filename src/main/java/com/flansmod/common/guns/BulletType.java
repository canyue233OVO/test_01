...
public class BulletType extends ShootableType
{
	...
	//2024 ammo weight system
	public int missileWeight = 50;
	
	//2024 pokemon evolution system
	public boolean moonstone = false;
	public String Evolution = "null"; //itemname of what target will evolve into
	public String PreEvolution = "null"; //itemname of the vehicle that can evolve from this moonstone    If i get less lazy maybe I can allow multiple vehicle to be listed here
	...
	//2024 screenshake effect
	public int powerShake = 0;
	public int rangeShake = 0;
	...
	//2024 let the plane handle the lock-on
	public boolean selfGuided = true; //let it be true by default so old misisle configs wont break
	...
	//copy o it just for drones so submunition wont suicide
	public boolean DroneDropped = false;
	...
	protected void read(String[] split, TypeFile file)
	{
		...
		try
		{
			...
			//2024 drone control
			else if(split[0].equals("TPVdrone"))
				TPVdrone = Boolean.parseBoolean(split[1]);
			else if(split[0].equals("droneSpeed"))
				droneSpeed = (float) (Float.parseFloat(split[1]));
			
			//2024 ammo weight system
			else if(split[0].equals("missileWeight"))
				missileWeight = (int) (Integer.parseInt(split[1]));
			
			//2024 for missiles that require the plane for guidance
			else if(split[0].equals("selfGuided"))
				selfGuided = Boolean.parseBoolean(split[1]);
			
			//2024 for pokemon evolution
			else if(split[0].equals("moonstone"))
				moonstone = Boolean.parseBoolean(split[1]);
			else if(split[0].equals("Evolution"))
				Evolution = split[1];
			else if(split[0].equals("PreEvolution"))
				PreEvolution = split[1];
			...
			//2024 so drone grenades dotn cause drone suicide
			else if(split[0].equals("DroneDropped"))
				DroneDropped = Boolean.parseBoolean(split[1]);
			...
			//2024 shake effect
			else if(split[0].equals("powerShake"))
				powerShake = Integer.parseInt(split[1]);
			else if(split[0].equals("rangeShake"))
				rangeShake = Integer.parseInt(split[1]);
