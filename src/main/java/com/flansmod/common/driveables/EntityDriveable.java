...
import com.flansmod.common.network.PacketMissileWeight;
import com.flansmod.common.network.PacketParticle;
import com.flansmod.common.network.PacketPlaySound;
import com.flansmod.common.network.PacketShakeRecoil;
...
public abstract class EntityDriveable extends Entity implements IControllable, IExplodeable, IEntityAdditionalSpawnData{ //weeder not there was a ", IUpdatable" after spawndata that was a wtf moment
	...
	//2024 for lockon so it wont spam losing target too easily
	public int lockLife = 0;
	 ...
	 */
	//public boolean locked = true; forget this shit
	...
	//2024 lockon plane-guided instead of bullet doing everything
	public Entity LockedTarget = null; //leave it true by default so all old missile configs will work
	public Entity ToggleLocked = null; //so you can switch targets
	//public Entity ToggleLocked2 = null; too hard to have 3 possible targets. the code is btfod
	...
	public void shootEach(DriveableType type, ShootPoint shootPoint, int currentGun, boolean secondary, EnumWeaponType weaponType, boolean evil) //the evil boolean indicates if its an ai ship auto firing
	{
		...
		if(weaponType == EnumWeaponType.SHELL || (weaponType == EnumWeaponType.BOMB && (type.secondaryDoor || type.primaryDoor)))
			isRecoil = true;
			...
			if (gunType != null ) //there was anull crash here, so applying extra null checker ivermectin
			{
				...
				if(bulletItemStack != null && bulletItemStack.getItem() instanceof ItemShootable && TeamsManager.bulletsEnabled && driveableData.overheatSuffer < 1)
				{
					...
					if(  (gunType.isAmmo(bullet) && !type.epicShip && ticksRepairing == 0) || type.epicShip && gunType.isAmmo(bullet) && (getDriveableData().parts.get(EnumDriveablePart.buoyancy).health)>3 && ticksFlareUsing <= 0 )
					{
						...
						//2025 allow coax gun to cause recoil
						if(type.coaxRecoil)
						{
						isRecoil = true;
						//System.out.println("p-please recoil: " + isRecoil);
						}
			...
			case BOMB :
			{
				if(TeamsManager.bombsEnabled)
				{
					...
					if(slot != -1)
					{
						...
						spawnParticle(type.shootParticle(secondary), shootPoint, gunVec);
							if(type.secondaryDoor || type.primaryDoor)
								isRecoil = true;
			...
			case SHELL :
			{
				...
				if(TeamsManager.shellsEnabled && !type.epicShip && ticksRepairing == 0 && driveableData.overheatSuffer < 1 || type.epicShip && (getDriveableData().parts.get(EnumDriveablePart.buoyancy).health)>3 && ticksFlareUsing <= 0 )
				{
					...
					//	System.out.println("evil? " + type.evilGolem);
						...
						//2024 first person vehicle recoil
						if(type.cannonRecoil > 1 && human != null)
							PacketShakeRecoil.sendShakePacket(posX, posY, posZ, type.cannonRecoil, 10, dimension); //the 10 is the range of the packet
							//FlansModClient.screenShaker(type.cannonRecoil); //uh oh this works in singlpelayer only
	...
	public void onUpdate(){
		...
		//2024 plane-guided missile lock on code
		//stealing lockon code from missiles, moving to plane
			if (this.ticksExisted % 5 == 0  && (type.lockOnToPlanes || type.lockOnToVehicles || type.lockOnToMechas || type.lockOnToLivings || type.lockOnToPlayers)) {				
				EntityPlayer Pilot = null;
				if (seats != null && seats[0] != null && seats[0].riddenByEntity instanceof EntityPlayer)
					Pilot = (EntityPlayer)seats[0].riddenByEntity;
				if (LockedTarget != null && Pilot!= null && Pilot instanceof EntityPlayerMP )
				{
					FlansMod.getPacketHandler().sendTo(new PacketFlak(LockedTarget.posX, LockedTarget.posY, LockedTarget.posZ, 1, "tankkuDeath"), (EntityPlayerMP)Pilot);		
				}				
				float fakeMotionX = 0;
				float fakeMotionY = -1f*this.axes.getPitch()/50f;
				float fakeMotionZ = 0;	
				if ((float)axes.getYaw() < 22.5 && (float)axes.getYaw() >= -22.5)//east
				{
					fakeMotionX = 1.2f;
					fakeMotionZ = 0f;
				}

				if ((float)axes.getYaw() < 67.5 && (float)axes.getYaw() >= 22.5) //SE
				{
					fakeMotionX = 0.6f;
					fakeMotionZ = 0.6f;
				}

				if ((float)axes.getYaw() >= 67.5 && (float)axes.getYaw() < 112.5)//souf
				{
					fakeMotionX = 0f;
					fakeMotionZ = 1.2f;
				}

				if ((float)axes.getYaw() >= 112.5 && (float)axes.getYaw() < 157.5) //SW
				{
					fakeMotionX = -0.6f;
					fakeMotionZ = 0.6f;
				}

				if ((float)axes.getYaw() < -157.5 || (float)axes.getYaw() >= 157.5) //west
				{
					fakeMotionX = -1.2f;
					fakeMotionZ = 0f;
				}

				if ((float)axes.getYaw() < -112.5 && (float)axes.getYaw() >= -157.5)//NW
				{
					fakeMotionX = -0.6f;
					fakeMotionZ = -0.6f;
				}

				if ((float)axes.getYaw() > -112.5 && (float)axes.getYaw() <= -67.5)//norf
				{
					fakeMotionX = 0f;
					fakeMotionZ = -1.2f;
				}

				if ((float)axes.getYaw() > -67.5 && (float)axes.getYaw() <= -22.5)//NE
				{
					fakeMotionX = 0.6f;
					fakeMotionZ = -0.6f;
				}
				...
				Vector3f motionVec = new Vector3f(fakeMotionX, fakeMotionY, fakeMotionZ); //if we use real motionXYZ then you can only lock while plane/tank is moving forwards
				Entity closestEntity = null;
				float closestAngle = type.maxLockOnAngle * 3.14159265F / 180F;
				//System.out.println("the motion vector: " + motionVec);
				
				for (Object obj : worldObj.loadedEntityList) {
					Entity entity = (Entity) obj;
					String etype = entity.getEntityData().getString("EntityType");   //preemptively removed living base because who the fuck would ever make a horse-seeking missile for planes. also reduce amount of targets to distract plane
					if ( !(entity instanceof EntityLivingBase) && ((type.lockOnToMechas && entity instanceof EntityMecha) ||
							...
							) {  //exclude togglelocked from candidate pool so they can be "Saved" and switched to
						...
						if ( Math.abs(missileDirection.x-targetDirection.x)<0.5f && Math.abs(missileDirection.y-targetDirection.y)<0.5f && Math.abs(missileDirection.z-targetDirection.z)<0.5f)
						{
							...
							BulletType currentMissile = null;
							boolean earlyinfrared = false;
							boolean antiRadiation = false;
							currentMissile = BulletType.getBullet(currentAmmo); //check if there is a missile loaded.
							if(currentMissile!=null && currentMissile.earlyInfrared) //if a missile is loaded, and its early infrared, that will affect planes lock on ability
								earlyinfrared = true;
							if(currentMissile!=null && currentMissile.antiRadiation) //if a missile is loaded, and its antiradiation, that will affect planes lock on ability
								antiRadiation = true;

							
						if (  entity != LockedTarget &&  ((angle < closestAngle && !earlyinfrared) || (angle < closestAngle && earlyinfrared && rearLock))   ) {  //used to be a check if not antirad in there but i quit on that for now to rpevent spaghetti code
							//if(driveableData.targetSwitchTimer>0 && LockedTarget!=entity || driveableData.targetSwitchTimer==0)
							//{
						...
						//for losing the lock
						if (lockLife < 5 && LockedTarget!= null && LockedTarget == entity && ( (angle > closestAngle && !earlyinfrared) || (angle > closestAngle && earlyinfrared || !rearLock) )) {
							if (Pilot != null)
							Pilot.addChatMessage(new ChatComponentText("Primary Target Lost"));
							
							LockedTarget = ToggleLocked;
						}
						
						/*  fuck it give up on anti rad for now
						if(entity instanceof EntityDriveable)
						{
							EntityDriveable victim = (EntityDriveable)entity;						
							//case for anti radiation
							if ((victim.getDriveableType().hasRadar) && victim.activeRadar && antiRadiation)
								{
									//if(driveableData.targetSwitchTimer>0 && LockedTarget!=entity || driveableData.targetSwitchTimer==0)
									//{
										closestEntity = entity;
							...
							//antirad target2
							if (entity!= LockedTarget && entity!= closestEntity && LockedTarget != null && (victim.getDriveableType().hasRadar) && victim.activeRadar && antiRadiation)
							{
								//if(driveableData.targetSwitchTimer>0 && LockedTarget!=entity || driveableData.targetSwitchTimer==0)
								//{
									closestEntity2 = entity;
									closestAngle = angle;
								//}	
								//System.out.println("successful anti rad lock, victims radar: " + victim.activeRadar);
							}	
				...
				if (closestEntity != null)
				{
					...
					ToggleLocked = closestEntity;
					
					if(closestEntity instanceof EntityDriveable && !worldObj.isRemote && Pilot!= null && closestEntity != LockedTarget  && ToggleLocked!= null  && LockedTarget == null)
					{
						EntityDriveable target = (EntityDriveable)closestEntity;
						Pilot.addChatMessage(new ChatComponentText("Successfully Locked Onto Target: " + target.getDriveableType().name));
						LockedTarget = ToggleLocked;
						ToggleLocked = closestEntity;
					}
				...
				if(LockedTarget == ToggleLocked)
				{
					ToggleLocked = null;
				}
				
						
			} //end of the %5 tick delayed lockon code block   2024
			
			if(lockLife>40) //buffer of time that lock stays active
				lockLife=40;
			else if(lockLife>0 && driveableData.targetSwitchTimer<6) //lock maintenance timer decays slower if you are spamming target switch
				lockLife--;
...
//lockOnSoundDelay--;
		...
		if (!worldObj.isRemote && type.numMissileSlots>0 && this.ticksExisted % 5 == 0 ) /*so it wont be spammed too much*/{		
			...
			if(slot != -1 && this.seats != null && this.seats[0] != null && seats[0].riddenByEntity != null && seats[0].riddenByEntity instanceof EntityPlayerMP){
				...
				//fuck it do the client version RIGHT FUCKING HERE  2024 ammo weight		    	
		        ...
		        if(this!=null) {
		            ...
		            if(this instanceof EntityPlane) {
		                ...
		                DriveableData dd = this.getDriveableData();
		                ...
		                if((dd.missiles).length>0) {
		                	//System.out.println("checking missiles");
		                	
		                    for (int i2 = 0; i2 < (dd.missiles).length; i2++)  {
		                        //i2++;
		                        if( dd.missiles[i2] != null && (dd.missiles[i2].getItem()) != null &&  (dd.missiles[i2].getItem()) instanceof ItemBullet && dd.missiles[i2].stackSize > 0)
		                        {
		                        	emptyChecker=1488; //if there is at LEAST one bullet in any bomb/missile slot, then its not empty
		                        ItemBullet ammoM = (ItemBullet)(dd.missiles[i2].getItem());
		                        int missileStackSize = dd.missiles[i2].stackSize; //sostacks of bombs/missiles wont weigh as much as just one bomb/missile
		               ...
		               // System.out.println("final weight tally is " + bodyfat + " and attempted assigned weight tally is " + dd.addedWeight);
		                
		    			//FlansMod.getPacketHandler().sendToServer(new PacketMissileWeight(this.getEntityId(), driveableData.getStackInSlot(slot), bodyfat)); //2024 ammo weight system
		                FlansMod.getPacketHandler().sendTo(new PacketMissileWeight(this.getEntityId(), bodyfat), (EntityPlayerMP)seats[0].riddenByEntity); //clientside version of same packet fucking defective   actually nvm its based now
		                	                
		            } 
		            
		            
		            
		        	//System.out.println("added weight: " + ((EntityDriveable)e).addedWeight + "   bodyfat: " + bodyfat);   lol dont put anything here since this area is outside of the block where its confirmed to be an entityplane
		        	//System.out.println("ioverwriting ammo: " + missile);
		            //((EntityDriveable)e).addedWeight = 1488;
		            
		            
		        }   //end of client "packet" moved right into the normal code 2024
			...
			if(emptyChecker == 0) //if no missiles, weight is zero
			{
				this.driveableData.addedWeight=0;
				  FlansMod.getPacketHandler().sendTo(new PacketMissileWeight(this.getEntityId(), 0), (EntityPlayerMP)seats[0].riddenByEntity);
			}
			//currentAmmo = ((ItemBullet)driveableData.getStackInSlot(slot).getItem()).type.name;


		}
		//baris ammo checker end
		...
		//2025
		if(rightMouseHeld && (type.secondaryDoor ||  type.coaxRecoil) && !disabled){
			tryRecoil();
			setRecoilTimer();
		}
		...
		if(TeamsManager.movieProp)
			throttle = 1; //command so vehicles drive themselves for video
		...
		{	
			driveableData.originX =  (int) this.posX; //when vehicle first born, save the coords. if there is a seatkick, you are going back here   9 4 23
			driveableData.originY =  (int) this.posY + 1;
			driveableData.originZ = (int) this.posZ;
		}	
			...
			if(((getDriveableData().parts.get(EnumDriveablePart.core).dead)&&(seat != null)&&type.inshallah==true)  ||   ((getDriveableData().parts.get(EnumDriveablePart.core).dead)&&(seat != null)&&type.canDive==true)    || (suicide==true&&(seat != null)&&type.inshallah==true))
			{
				...
				if(!worldObj.isRemote)
				{
					...
					{FlansMod.proxy.spawnParticle("flansmod.FMNuke",
							...
							0,0,0);
					PacketPlaySound.sendSoundPacket(posX, posY, posZ, 250, dimension, "YamatoGun", false);
					PacketShakeRecoil.sendShakePacket(posX, posY, posZ, 250, 120, dimension);}
					if(type.bigDeath==true)
					{FlansMod.proxy.spawnParticle("flansmod.shipDeath",
							...
							0,0,0);
					PacketPlaySound.sendSoundPacket(posX, posY, posZ, 250, dimension, "BattleshipGun", false);
					PacketShakeRecoil.sendShakePacket(posX, posY, posZ, 120, 80, dimension);}
					if(type.Death==true)
					{FlansMod.proxy.spawnParticle("flansmod.tankDeath",
							...
							0,0,0);
					PacketPlaySound.sendSoundPacket(posX, posY, posZ, 250, dimension, "explcls6", false);
					PacketShakeRecoil.sendShakePacket(posX, posY, posZ, 88, 69, dimension);}
					for(DriveablePart part : driveableData.parts.values())
					{
						if(part.health > 0 && !part.dead)
							killPart(part);
	...
	private void killPart(DriveablePart part)
	{
		...
		ItemStack grenadeStack = InfoType.getRecipeElement(type.turretGrenade, 1); 
		Item corpse = grenadeStack.getItem();

		
		EntityPlayer Master = null;
		if(this.seats[0] != null && this.seats[0].riddenByEntity != null && this.seats[0].riddenByEntity instanceof EntityPlayer)
		{
			Master = (EntityPlayer)this.seats[0].riddenByEntity;
			//System.out.println("master set: " + Master);
		}
	//	System.out.println("trying to die");
		EntityGrenade grenade = new EntityGrenade(this.worldObj, (((ItemGrenade)corpse).type), Master, this);  
		
		//System.out.println("the part: " + part.type);
		
		//System.out.println("weakspot is dead? " + getDriveableData().parts.get(EnumDriveablePart.turretWeak).dead);
		
		//Spawn the entity server side
		
		//modified for weakspot killed turrets only
		if (type.turretTossTrue && grenade != null && part.type == EnumDriveablePart.turret &&  ( getDriveableData().parts.get(EnumDriveablePart.turretWeak).dead || getDriveableData().parts.get(EnumDriveablePart.turretWeak2).dead )) 
		{
		if(!this.worldObj.isRemote)
		{
			{FlansMod.proxy.spawnParticle("flansmod.tankDeath", //also do a tankdeath explosion
					this.posX,
					this.posY,
					this.posZ,
					0,0,0);
			...
			//FlansMod.proxy.spawnParticle("flansmod.TurretTest", //failed attempt at 3D turret particle effect
			//		this.posX,
			//		this.posY,
			//		this.posZ,
			//		2*(Math.random()-0.5f),10,2*(Math.random()-0.5f));
			
			//System.out.println("trying to spawn death grenade");
			this.worldObj.spawnEntityInWorld(grenade);
			PacketShakeRecoil.sendShakePacket(posX, posY, posZ, 88, 69, dimension); //send a big screen shake to everyone witnessing the tanks death
		}
		}//end of turret only if barrier
	...
	*/
	...
	public void tryRecoil()
	{
		...
		if((recoilTimer <= 0 && (slot != -1 || (type.secondaryDoor || type.primaryDoor))))
			isRecoil = true;
	}
	...
	public void setRecoilTimer()
	{
		...
		if(recoilTimer <= 0 && (slot != -1 || (type.secondaryDoor || type.primaryDoor)))
		{
			...
			if (getDriveableType().secondaryDoor)
			recoilTimer = getDriveableType().shootDelaySecondary + 20;
			if (getDriveableType().primaryDoor)
			recoilTimer = getDriveableType().shootDelayPrimary + 20;
