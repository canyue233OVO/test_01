    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        switch (cmd) {
            ...
            */
            ...
            case "claimzone":{
                if (args.length > 2)
                    cmdClaimZone(sender, args[1], args.length > 2 ? args[2] : "未命名区域");
            }break;
            
            case "unclaimzone":{
                cmdUnclaimZone(sender);
            }break;
            ...
            case "decision":{
                if (args.length > 1)
                    cmdDecision(sender, args[1]);
                else
                    cmdDecisionList(sender);
            }break;
            ...
            case "produce":{
                if (args.length > 2)
                    cmdProduce(sender, args[1], args[2]);
                else
                    cmdProduceList(sender);
            }break;
            ...
            case "population":{
                cmdPopulation(sender);
            }break;
            
            case "collect":{
                cmdCollectProduction(sender);
            }break;
...
*/
     ...
     * 宣称区块为指定类型
     */
    private void cmdClaimZone(ICommandSender sender, String zoneTypeStr, String zoneName) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        if(clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有领袖和官员可以宣称区域！"));
            return;
        }
        ...
        ClowderTerritory.TerritoryMeta meta = ClowderTerritory.getMetaFromIntCoords(x, z);
        if(meta == null || meta.owner.owner != clowder) {
            sender.addChatMessage(new ChatComponentText(ERROR + "你必须站在己方旗帜控制的区域！"));
            return;
        }
        
        ZoneType zoneType = ZoneType.fromString(zoneTypeStr);
        if(zoneType == null) {
            sender.addChatMessage(new ChatComponentText(ERROR + "无效的区域类型！可用：residential, mining, administrative, industrial, research"));
            return;
        }
        
        ClaimedZone existing = clowder.getClaimedZone(meta.flagX, meta.flagY, meta.flagZ);
        if(existing != null) {
            sender.addChatMessage(new ChatComponentText(ERROR + "该旗帜已被宣称为" + existing.zoneType.getDisplayName() + "！"));
            return;
        }
        
        if(clowder.claimZone(zoneType, meta.flagX, meta.flagY, meta.flagZ, zoneName, player.worldObj)) {
            sender.addChatMessage(new ChatComponentText(TITLE + "成功将区域宣称为" + zoneType.getDisplayName() + "！"));
            clowder.notifyAll(player.worldObj, new ChatComponentText(INFO + player.getDisplayName() + " 宣称了一个" + zoneType.getDisplayName() + "：" + zoneName));
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "宣称失败！资金不足。需要：" + ZoneConfig.getConfig(zoneType).buildCost));
        }
    }
    
    private void cmdUnclaimZone(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        if(clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有领袖和官员可以取消区域宣称！"));
            return;
        }
        ...
        if(clowder.unclaimZone(meta.flagX, meta.flagY, meta.flagZ, player.worldObj)) {
            sender.addChatMessage(new ChatComponentText(TITLE + "成功取消区域宣称！"));
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "该区域未被宣称！"));
        }
    }
    
    private void cmdZoneInfo(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        ClowderTerritory.TerritoryMeta meta = ClowderTerritory.getMetaFromIntCoords(x, z);
        if(meta == null) {
            sender.addChatMessage(new ChatComponentText(INFO + "当前区域：荒野"));
            return;
        }
        
        if(meta.owner.owner != clowder) {
            sender.addChatMessage(new ChatComponentText(INFO + "当前区域：" + meta.owner.owner.name + " 的领土"));
            return;
        }
        
        ClaimedZone zone = clowder.getClaimedZone(meta.flagX, meta.flagY, meta.flagZ);
        if(zone == null) {
            sender.addChatMessage(new ChatComponentText(INFO + "当前区域：己方领土（未宣称功能区）"));
            return;
        }
        
        sender.addChatMessage(new ChatComponentText(TITLE + "===== 区域信息 ====="));
        sender.addChatMessage(new ChatComponentText(INFO + "区域名称：" + zone.zoneName));
        sender.addChatMessage(new ChatComponentText(INFO + "区域类型：" + zone.zoneType.getDisplayName()));
        sender.addChatMessage(new ChatComponentText(INFO + "区域等级：" + zone.level));
        
        if(zone.zoneType == ZoneType.RESIDENTIAL) {
            sender.addChatMessage(new ChatComponentText(INFO + "存储人力：" + String.format("%.1f", zone.storedManpower)));
            sender.addChatMessage(new ChatComponentText(INFO + "存储适役人口：" + String.format("%.1f", zone.storedEligiblePop)));
        } else if(zone.zoneType == ZoneType.ADMINISTRATIVE) {
            sender.addChatMessage(new ChatComponentText(INFO + "存储人力：" + String.format("%.1f", zone.storedManpower)));
            sender.addChatMessage(new ChatComponentText(INFO + "存储政治点数：" + String.format("%.1f", zone.storedPoliticalPower)));
        } else if(zone.zoneType == ZoneType.RESEARCH) {
            sender.addChatMessage(new ChatComponentText(INFO + "存储科研点数：" + String.format("%.1f", zone.storedResearchPoints)));
        } else if(zone.zoneType == ZoneType.INDUSTRIAL) {
            sender.addChatMessage(new ChatComponentText(INFO + "民用工厂：" + zone.civilianFactories));
            sender.addChatMessage(new ChatComponentText(INFO + "军用工厂：" + zone.militaryFactories));
        }
    }
    
    private void cmdListZones(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "===== 派系区域列表 ====="));
        
        if(clowder.claimedZones.isEmpty()) {
            sender.addChatMessage(new ChatComponentText(INFO + "尚未宣称任何功能区域"));
            return;
        }
        
        int residential = 0, mining = 0, administrative = 0, industrial = 0, research = 0;
        for(ClaimedZone zone : clowder.claimedZones.values()) {
            switch(zone.zoneType) {
                case RESIDENTIAL: residential++; break;
                case MINING: mining++; break;
                case ADMINISTRATIVE: administrative++; break;
                case INDUSTRIAL: industrial++; break;
                case RESEARCH: research++; break;
            }
        }
        
        sender.addChatMessage(new ChatComponentText(INFO + "居民区：" + residential));
        sender.addChatMessage(new ChatComponentText(INFO + "矿产区：" + mining));
        sender.addChatMessage(new ChatComponentText(INFO + "行政区：" + administrative));
        sender.addChatMessage(new ChatComponentText(INFO + "工业区：" + industrial));
        sender.addChatMessage(new ChatComponentText(INFO + "科研区：" + research));
        sender.addChatMessage(new ChatComponentText(INFO + "总计：" + clowder.claimedZones.size() + " 个功能区"));
    }
    ...
    private void cmdStorehouse(ICommandSender sender) {
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "===== 派系仓库 ====="));
        sender.addChatMessage(new ChatComponentText(INFO + "仓库物品总数：" + clowder.storehouse.getTotalItemCount()));
        sender.addChatMessage(new ChatComponentText(INFO + "使用 /c collect 收集所有区域产出"));
    }
    ...
    private void cmdPolicy(ICommandSender sender) {
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "===== 国策树 ====="));
        sender.addChatMessage(new ChatComponentText(INFO + "意识形态：" + clowder.policyTree.ideology));
        sender.addChatMessage(new ChatComponentText(INFO + "已解锁国策：" + clowder.policyTree.unlockedPolicies.size()));
        
        if(clowder.policyTree.currentResearch != null) {
            sender.addChatMessage(new ChatComponentText(INFO + "正在研究：" + clowder.policyTree.currentResearch));
            sender.addChatMessage(new ChatComponentText(INFO + "研究进度：" + clowder.policyTree.researchProgress + " 天"));
        } else {
            sender.addChatMessage(new ChatComponentText(INFO + "当前未研究任何国策"));
        }
    }
    
    private void cmdDecision(ICommandSender sender, String decisionId) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        if(clowder.getPermLevel(player.getDisplayName()) < 3) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有领袖可以激活决议！"));
            return;
        }
        
        if(clowder.decisionSystem.activateDecision(decisionId)) {
            sender.addChatMessage(new ChatComponentText(TITLE + "成功激活决议：" + decisionId));
            clowder.notifyAll(player.worldObj, new ChatComponentText(INFO + "领袖激活了决议：" + decisionId));
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "激活决议失败！可能是政治点数不足、决议冷却中或已达到最大激活数量。"));
        }
    }
    
    private void cmdDecisionList(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "===== 决议系统 ====="));
        sender.addChatMessage(new ChatComponentText(INFO + "政治点数：" + String.format("%.1f", clowder.getPoliticalPower())));
        sender.addChatMessage(new ChatComponentText(INFO + "激活的决议数量：" + clowder.decisionSystem.activeDecisions.size() + "/" + DecisionSystem.MAX_ACTIVE_DECISIONS));
        
        for(DecisionSystem.ActiveDecision active : clowder.decisionSystem.activeDecisions) {
            sender.addChatMessage(new ChatComponentText(INFO + "- " + active.decisionId + " (剩余" + active.remainingDays + "天)"));
        }
    }
    
    private void cmdTech(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(INFO + "科研点数：" + String.format("%.1f", clowder.getResearchPoints())));
        sender.addChatMessage(new ChatComponentText(INFO + "已解锁科技：" + clowder.technologyTree.unlockedTechs.size()));
        
        if(clowder.technologyTree.currentResearch != null) {
            sender.addChatMessage(new ChatComponentText(INFO + "正在研究：" + clowder.technologyTree.currentResearch));
            sender.addChatMessage(new ChatComponentText(INFO + "研究进度：" + String.format("%.1f", clowder.technologyTree.researchProgress)));
        } else {
            sender.addChatMessage(new ChatComponentText(INFO + "当前未研究任何科技"));
        }
    }
    
    private void cmdProduce(ICommandSender sender, String projectId, String quantityStr) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        if(clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有领袖和官员可以管理生产！"));
            return;
        }
        
        try {
            int quantity = Integer.parseInt(quantityStr);
            if(clowder.productionSystem.startProduction(projectId, quantity)) {
                sender.addChatMessage(new ChatComponentText(TITLE + "成功开始生产：" + projectId + " x" + quantity));
            } else {
                sender.addChatMessage(new ChatComponentText(ERROR + "开始生产失败！可能是科技未解锁或生产槽位已满。"));
            }
        } catch(NumberFormatException e) {
            sender.addChatMessage(new ChatComponentText(ERROR + "无效的数量！"));
        }
    }
    
    private void cmdProduceList(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(INFO + "生产槽位：" + clowder.productionSystem.getProductionQueues().size() + "/" + clowder.productionSystem.getMaxProductionSlots()));
        
        int index = 1;
        for(ProductionSystem.ProductionQueue queue : clowder.productionSystem.getProductionQueues()) {
            sender.addChatMessage(new ChatComponentText(INFO + index + ". " + queue.projectId + " - 剩余：" + queue.remainingQuantity + " 进度：" + String.format("%.1f%%", queue.progress * 100)));
            index++;
        }
    }
    ...
    private void cmdFactory(ICommandSender sender, String factoryType) {
        ...
        if(clowder.getPermLevel(player.getDisplayName()) < 2) {
            sender.addChatMessage(new ChatComponentText(ERROR + "只有领袖和官员可以建造工厂！"));
            return;
        }
        ...
        ClowderTerritory.TerritoryMeta meta = ClowderTerritory.getMetaFromIntCoords(x, z);
        if(meta == null || meta.owner.owner != clowder) {
            sender.addChatMessage(new ChatComponentText(ERROR + "你必须站在己方领土上！"));
            return;
        }
        
        ClaimedZone zone = clowder.getClaimedZone(meta.flagX, meta.flagY, meta.flagZ);
        if(zone == null || zone.zoneType != ZoneType.INDUSTRIAL) {
            sender.addChatMessage(new ChatComponentText(ERROR + "你必须站在工业区才能建造工厂！"));
            return;
        }
        
        boolean isMilitary = factoryType.equalsIgnoreCase("military") || factoryType.equalsIgnoreCase("军用");
        float cost = isMilitary ? 5000.0f : 3000.0f;
        
        if(zone.buildFactory(isMilitary, cost)) {
            String typeStr = isMilitary ? "军用工厂" : "民用工厂";
            sender.addChatMessage(new ChatComponentText(TITLE + "成功建造" + typeStr + "！"));
            clowder.notifyAll(player.worldObj, new ChatComponentText(INFO + player.getDisplayName() + " 建造了一个" + typeStr));
        } else {
            sender.addChatMessage(new ChatComponentText(ERROR + "建造失败！资金不足。需要：" + cost));
        }
    }
    
    private void cmdPopulation(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        sender.addChatMessage(new ChatComponentText(TITLE + "===== 人口统计 ====="));
        sender.addChatMessage(new ChatComponentText(INFO + "总人口：" + String.format("%.0f", clowder.populationSystem.getPopulation())));
        sender.addChatMessage(new ChatComponentText(INFO + "人力：" + String.format("%.1f", clowder.populationSystem.getManpower())));
        sender.addChatMessage(new ChatComponentText(INFO + "适役人口：" + String.format("%.1f", clowder.populationSystem.getEligiblePopulation())));
        sender.addChatMessage(new ChatComponentText(INFO + "政治点数：" + String.format("%.1f", clowder.getPoliticalPower())));
        sender.addChatMessage(new ChatComponentText(INFO + "科研点数：" + String.format("%.1f", clowder.getResearchPoints())));
    }
    
    private void cmdCollectProduction(ICommandSender sender) {
        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Clowder clowder = Clowder.getClowderFromPlayer(player);
        ...
        clowder.collectAllZoneProduction();
        sender.addChatMessage(new ChatComponentText(TITLE + "已收集所有区域产出！"));
        sender.addChatMessage(new ChatComponentText(INFO + "人力：" + String.format("%.1f", clowder.populationSystem.getManpower())));
        sender.addChatMessage(new ChatComponentText(INFO + "适役人口：" + String.format("%.1f", clowder.populationSystem.getEligiblePopulation())));
        sender.addChatMessage(new ChatComponentText(INFO + "政治点数：" + String.format("%.1f", clowder.getPoliticalPower())));
        sender.addChatMessage(new ChatComponentText(INFO + "科研点数：" + String.format("%.1f", clowder.getResearchPoints())));
