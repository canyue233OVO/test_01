...
public class EntityVehicle extends EntityDriveable implements IExplodeable
{
	...
	//for pokemon evolution 2024
	public int dynamicScale = 1;
...
//						((EntityPlayer)seats[i].riddenByEntity).addPotionEffect(new PotionEffect(Potion.harm.id, 10, 5));
		...
		System.out.println("ejection seat button worked");
			...
			case 10 : //Change control mode : surface for subs
			{
				...
				{
                FlansMod.proxy.changeControlMode((EntityPlayer)this.seats[0].riddenByEntity); //fuck it let all tanks use it now that its fixed(?) june 16 2023
			...
			case 11 : //Roll left : Do nothing i mean lock turret       actually do nothing june 16 2023   old mouse control is so back!
			{
				//if (!(type.walterGunRange || type.walterMortar) && !type.gunRange)
				//seats[0].seatInfo.aimingSpeed = new Vector3f(0f, 0f, 0f);
				return true;
			}
			case 12 : //Roll right : Do nothing i mean unlock turret   actually do nothing june 16 2023   old mouse control is so back!
			{
				//if (!(type.walterGunRange || type.walterMortar) && !type.gunRange)
			...
			case 14 : // Door
			{
				//if(worldObj.isRemote) //manual seatkick simulator
	...
	//	this.renderDistanceWeight = 4000.0D; goldsloth code says remove this
		...
		//for pokemon evolution animation 2024
		if(Evolving)
		{
			if(dynamicScale > 1)
				dynamicScale--;
			if(dynamicScale > 5)
			FlansMod.getPacketHandler().sendToAllAround(new PacketParticle("enchantmenttable", posX, posY+1, posZ, (float)Math.random(), (float)Math.random(), -(float)Math.random()), posX, posY+1, posZ, 88, worldObj.provider.dimensionId);
		
			if(dynamicScale <= 1)
				evolution(evolution); //load the name of what you will evolve into once animation is done
}
		if(ticksExisted < 40 && dynamicScale < 100)
		{
			dynamicScale+= 1; //when vehicle is first born, have a cute scale up animation but also do it in reverse for pokemon evolution
			if(dynamicScale < 40)
			{
				dynamicScale+= 4;
				FlansMod.getPacketHandler().sendToAllAround(new PacketParticle("heart", posX, posY, posZ, (float)Math.random(), (float)Math.random(), -(float)Math.random()), posX, posY+1, posZ, 15, worldObj.provider.dimensionId);
			}
			if(dynamicScale < 90)
				dynamicScale+= 2;
			if(dynamicScale < 70)
	...
	setDead();
		...
		if ((getDriveableData().parts.get(EnumDriveablePart.turretWeak).maxHealth)>0 && (getDriveableData().parts.get(EnumDriveablePart.turretWeak).health)<=0 && (getDriveableData().parts.get(EnumDriveablePart.turret).health)>0 || (getDriveableData().parts.get(EnumDriveablePart.turretWeak2).maxHealth)>0 && (getDriveableData().parts.get(EnumDriveablePart.turretWeak2).health)<=0 && (getDriveableData().parts.get(EnumDriveablePart.turret).health)>0)
		{
			...
			//attackPart(EnumDriveablePart.turret, DamageSource.cactus, 1+(float)(getDriveableData().parts.get(EnumDriveablePart.turret).maxHealth)/(float)(4*type.WeakspotCookTime));
				...
				checkParts(); //make sure turret admits its dead so it can do deturret animation 2024
...
if(this.nintendoSwitchLite < 10)
	...
	//	}
		...
		for(EntityWheel wheel : wheels)
		{
			...
			//Apply velocity
			//If the player driving this is in creative, then we can thrust, no matter what         also it can thrust itself if its a self driving movie prop
			boolean canThrustCreatively = TeamsManager.movieProp || !TeamsManager.vehiclesNeedFuel || (seats != null && seats[0] != null && seats[0].riddenByEntity instanceof EntityPlayer && ((EntityPlayer)seats[0].riddenByEntity).capabilities.isCreativeMode);
...
//submarine
		...
		for(EntitySeat seat : seats)
		{
			...
			{   //use variable teams manager packet rate for non empty seat,    else empty seat 20x slower
				seat.updatePosition();
	        }	
		...
		if(thePlayerIsDrivingThis && ticksExisted % TeamsManager.vehicleRate == 0)
		{
			FlansMod.getPacketHandler().sendToServer(new PacketVehicleControl(this));
			serverPosX = posX;
			serverPosY = posY;
		...
		if(!worldObj.isRemote && ticksExisted % TeamsManager.vehicleRate == 0) //9 4 2023 improved icarus system
		{
		...
		int animSpeed = 10; //2025 decreased speed
		...
		if(throttle > 0.05){
			...
			animCountLeft ++;
			animCountRight --;
			animSpeed = 10; //2025 slowed down from 1 to 10  so when turning in place, no hyper speed leg animation
			...
			animCountLeft --;
			animCountRight ++;
			animSpeed = 10; //2025 slowed down from 1 to 10
			if (soundPosition == 0 && hasEnoughFuel() && type.tank)
			{
				PacketPlaySound.sendSoundPacket(posX, posY, posZ, type.engineSoundRange, dimension, type.engineSound, false);
				soundPosition = type.engineSoundLength;
  ...
  //for evil ai ships or CIWS?!
    ...
    public void evolution(String evolution)   //2024 pokemon evolution upgrade system    input is name of the vehicle you will evolve into
    {
    	 ItemStack evolutionStack = InfoType.getRecipeElement(evolution, 1, 0);	//the 0 is because of paintjob cringe or else server-killer world corrupter

    	DriveableType Ack = DriveableType.getDriveable(evolution); //to get the type from the shortname string   	
 		NBTTagCompound tags = new NBTTagCompound(); //all this stupid shit i had to copy over or else game crashes when spawning a raw vehicle item
 		tags.setString("Engine", ("FlightModel")); //default engine name is hardcoded into mod now so it will fuck you over if you remove labjac engine item :)   
 		tags.setString("Type", evolution);
     	for(EnumDriveablePart part : EnumDriveablePart.values())
     	{
     		tags.setInteger(part.getShortName() + "_Health", Ack.health.get(part) == null ? 0 : Ack.health.get(part).health);
     		tags.setInteger(part.getShortName() + "_Crew", Ack.crew.get(part) == null ? 0 : Ack.crew.get(part).crew);
     		tags.setBoolean(part.getShortName() + "_Fire", false);
     	}
     	evolutionStack.stackTagCompound = tags;


    	 Item evolutionTarget = evolutionStack.getItem();
    	 
    	 if(evolutionTarget instanceof ItemVehicle) //dont evolve into a nonexistent vehicle
    	 {
    	 ((ItemVehicle)evolutionStack.getItem()).spawnVehicleAngled(worldObj, this.posX + 0.5F, this.posY + 0.5F, this.posZ + 0.5F, axes.getYaw(), evolutionStack);
    		//EntityGrenade grenade = new EntityGrenade(this.worldObj, (((ItemGrenade)corpse).type), Master, this);  
    		
			FlansMod.getPacketHandler().sendToAllAround(new PacketFlak(posX, posY+1, posZ, 7, "smallSmoke"), posX, posY, posZ, 200, this.dimension);
	...
	public void reset(EntityPlayer master) //9 4 23   jus give the vehicle item back to player, then tp them to origin point. then plane commits suicide
	{
		...
		if(used && master != null)
		{
		MinecraftServer.getServer().getCommandManager().executeCommand(MinecraftServer.getServer(), "tp " + master.getDisplayName() + " " + driveableData.originX + " " + driveableData.originY + " " + driveableData.originZ);
		
		FlansMod.getPacketHandler().sendToAllAround(new PacketFlak(posX, posY, posZ, 1, "tankkuDeath"), posX, posY, posZ, 150, dimension);
		PacketPlaySound.sendSoundPacket(posX, posY, posZ, 150, dimension, "Crash", false);
		PacketPlaySound.sendSoundPacket(posX, posY, posZ, 150, dimension, "explcls6", false);
		
		master.addPotionEffect(new PotionEffect(Potion.resistance.id, 200, 9000)); //to survive fall damage after warp
		master.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 200, 9000)); //but also to prevent combat abuse, immobilize and blind plane survivor 
		master.addPotionEffect(new PotionEffect(Potion.blindness.id, 100, 9000));
		
		
		for(EntitySeat seat : seats) //now die!
			if(seat != null)
				seat.setDead();
		for(EntityWheel wheel : wheels)
			if(wheel != null)
...
//if(!type.vanillaDamage && (damagesource.damageType.equals("arrow") || damagesource.damageType.equals("thrown") || damagesource.damageType.equals("indirectMagic")|| damagesource.damageType.equals("player")|| damagesource.damageType.equals("fireball")|| damagesource.damageType.equals("mob")|| damagesource.damageType.equals("indirectMagic")) )
		...
		if(damagesource.damageType.equals("player") && damagesource.getEntity().onGround && (seats[0] == null || seats[0].riddenByEntity == null) && !(owner != Factions.getFactionFromPlayer((EntityPlayer) damagesource.getEntity())) && !type.unpunchable)
		{
		     //forget master
			driveableData.seatBelt = "null";
