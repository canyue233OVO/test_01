...
public class ModelVehicle extends ModelDriveable
{
	...
	public ModelRendererTurbo rightAnimTrackModel4[] = new ModelRendererTurbo[0]; //2025 added 4th frame
	public ModelRendererTurbo leftAnimTrackModel4[] = new ModelRendererTurbo[0];
	...
	public void render(DriveableType type)
	{
		...
		renderPart(rightAnimTrackModel4); //2025
		renderPart(leftAnimTrackModel4);
	...
	public void renderTurretLoss(DriveableType type) //2024 for grenade gore turret loss
	{
		super.render(type);
		GL11.glPushMatrix();
		GL11.glScalef(100*turretScale.x, 100*turretScale.y, 100*turretScale.z);  //if you cant stop body from rendering, just make whole thing 100x smaller, except turret!!!
		GL11.glTranslatef(turretTrans.x, turretTrans.y, turretTrans.z);
		renderPart(turretModel);
		renderPart(barrelModel);
		GL11.glPopMatrix();
	...
	public void render(float f5, EntityVehicle vehicle, float f)
    {
		...
		if(vehicle.isPartIntact(EnumDriveablePart.leftTrack))
		{
			...
			} else if (vehicle.animFrameLeft == 3){ //2025 added 4th frame
				for (ModelRendererTurbo aLeftTrackModel4 : leftAnimTrackModel4) {
					aLeftTrackModel4.render(f5, oldRotateOrder);
				}
			}
		...
		if(vehicle.isPartIntact(EnumDriveablePart.rightTrack))
		{
			...
			} else if (vehicle.animFrameRight == 3){ //2025 added 4th frame
				for (ModelRendererTurbo aRightTrackModel4 : rightAnimTrackModel4) {
					aRightTrackModel4.render(f5, oldRotateOrder);
				}
			}	
	...
	public void flipAll()
	{
		...
		flip(rightAnimTrackModel4); //2025
		flip(leftAnimTrackModel4);
		flip(drakonArmModel);
	...
	public void translateAll(float x, float y, float z)
	{
		...
		translate(rightAnimTrackModel4, x, y, z); //added 4th frame 2025
		translate(leftAnimTrackModel4, x, y, z);
		for(ModelRendererTurbo[] latm : leftAnimTrackModel)
