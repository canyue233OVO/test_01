...
public class EntityGrenade extends EntityShootable implements IEntityAdditionalSpawnData
{
	...
	public String MasterVehicle = "altayUrban";
	...
	//vehicle parts added for vehicle gore 2024
	public EntityGrenade(World w, GrenadeType g, EntityLivingBase t, EntityDriveable V)
	{
		this(w);

		if(V!=null) //2024 option to spawn from vehicle
		{
			setPosition(V.posX, V.posY + V.getEyeHeight(), V.posZ);
			MasterVehicle = V.getDriveableType().shortName;
		}
		else if(t!=null)
		...
		thrower = t;
		if(thrower != null && thrower instanceof EntityPlayer && PlayerHandler.getPlayerData((EntityPlayer)thrower) != null)
		{
		...
		if(t!=null) //2024 option to spawn from vehicle
		{
			x = t.rotationYaw;
			z = t.rotationPitch;
		}
		
		VehicleType selectedType = VehicleType.getVehicle(MasterVehicle);
		System.out.println("type: " + selectedType);
		System.out.println("Master: " + MasterVehicle);
		System.out.println("The entity: " + V);
		...
		if(worldObj.isRemote && selectedType != null && selectedType instanceof VehicleType && type.vehicleReference!= null)
		{
			...
			{
				System.out.println("cmon render buddy");
		...
		//added by labjac just for corpse physics xdd
		if(g.randomDirection)
		{
			//System.out.println("trying to be random");
			//axes.setAngles((float)(360*Math.random()), g.spinWhenThrown ? (float)(-135f - 44*Math.random()) : 0F, 0F); original number for human gore
			axes.setAngles((float)(360*Math.random()), (float)(-66 + -22*Math.random()), 90F);
		}
		else
		{
		...
		axes.setAngles(x + 90F, g.spinWhenThrown ? z : 0F, 0F);
		}
		rotationYaw = prevRotationYaw = g.spinWhenThrown ? x + 90F : 0F;
		rotationPitch = prevRotationPitch = z;
		//Give the grenade velocity in the direction the player is looking
		float speed = 0.5F * type.throwSpeed;
		if(g.randomDirection)
			speed *= (1.5f*Math.random());//also make speed more random
		motionX = axes.getXAxis().x * speed;
		motionY = axes.getXAxis().y * speed * 1.5;
