...
//labjac mount and blade
	...
	//labjac multikill counter  june 23 2023
	public int killStreak = 0;
	public int killStreakTimer = 0;
	
	//for kill sound  july 1 2023
	public int died = 0;
	...
	{
		if (killStreak > 0) //kill streak timers june 23 2023
			killStreakTimer++;
		if(killStreakTimer > 80) //you have 4 seconds before kill streak timer resets 
		{
			killStreak = 0;
			killStreakTimer = 0;
		}
		...
		if(died > 0)  //funny death noise july 1 2023
		{
			int random = 0;
        ...
        if(player != null)
        {
        	FlansMod.getPacketHandler().sendToAllAround(new PacketPlaySound(player.posX, player.posY, player.posZ, sound, false, false), player.posX, player.posY, player.posZ, 10f, player.dimension);
        	FlansMod.getPacketHandler().sendToAllAround(new PacketPlaySound(player.posX, player.posY, player.posZ, sound, false, false), player.posX, player.posY, player.posZ, 25f, player.dimension);
        }
        died = 0;
		}
    
		
		 //kill sound june 23 2023
        if((PlayerHandler.getPlayerData(player)).killStreakTimer == 10)
        {
        	if((PlayerHandler.getPlayerData(player)).killStreak == 1)
        	PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 3f, player.dimension, "killSound", false);
        	
        	else if((PlayerHandler.getPlayerData(player)).killStreak > 1)
        	PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 5f, player.dimension, "killStreak" + killStreak, false);
        }
