...
public class ModelCustomArmour extends ModelBiped 
{
	...
	{ 
		GL11.glPushMatrix();
		//GL11.glScalef(type.modelScale, type.modelScale, type.modelScale); attempt 3 to fix
		isSneak = entity.isSneaking();
		ItemStack itemstack = ((EntityLivingBase)entity).getEquipmentInSlot(0);
        heldItemRight = itemstack != null ? 1 : 0;
        
        
        //invisibility armor hide -t labjac
        if (entity.isInvisible() && !type.playermodel)
        	GL11.glScalef(0.0001f, 0.0001f, 0.0001f);
        
        //attempt 2
        if (entity != null) //import entity seat too
        {
        	EntityDriveable vehicle = null;
        	if(entity.isRiding() && entity.ridingEntity instanceof EntitySeat && ((EntitySeat)entity.ridingEntity).driveable != null)
        		vehicle = ((EntitySeat)entity.ridingEntity).driveable;
        	else vehicle = null;
        	
        	if(vehicle != null && vehicle.getDriveableType() != null && vehicle.getDriveableType().setPlayerInvisible)
        		GL11.glScalef(0.0001f, 0.0001f, 0.0001f);
        	else
