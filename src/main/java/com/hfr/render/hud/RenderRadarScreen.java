...
public class RenderRadarScreen {
	...
	public static void renderBlips(int marginX, int marginY, int size, double zLevel, float blipSize, float clamp, int range) {
		...
		for(Blip blip : blips) {
			...
			{
				//changed to display range to target 2025
				minecraft.fontRenderer.drawString("Range", (int) (cX * fontScale + (int)(x * clamp * fontScale) - 12), (int) (cY * fontScale + (int)(z * clamp * fontScale) + 4), 0xFFBBBB);
				minecraft.fontRenderer.drawString("" + Math.round(blip.Distance), (int) (cX * fontScale + (int)(x * clamp * fontScale) - 12), (int) (cY * fontScale + (int)(z * clamp * fontScale) + 12), 0xFFBBBB);
	...
	public static class Blip {
		...
		public Blip(float x, float y, float z, float posX, float posZ, int type, float Distance) {
			this.x = x;
			this.y = y;
			this.z = z;
			this.posX = posX;
