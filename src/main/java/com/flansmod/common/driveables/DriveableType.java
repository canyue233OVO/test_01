...
public class DriveableType extends PaintableType{
	...
	//2025 level of detail experiment
	public ModelDriveable modelShitty;
	...
	public boolean missilePrimaryAuto = false; //2024 improved version that automatically places model based on shootpoints
	public boolean missileSecondaryAuto = false;
	
    //2025 door auto opens when firing weapon, for plane missile doors
    public boolean primaryDoor = false;
    public boolean secondaryDoor = false;
	...
	//2025 tier shit
	public float tier = -10;
	public float expValue = 50;
	public float repairCost = 1488;
	...
	//2024 turret animation
	public boolean turretTossTrue = false;
	public String	turretGrenade		= "turretGore";
	...
	public float maxLockOnAngle = 120; //2024 letting plane guide the missiles
	
	//2024 for nu first person vehicle recoil
	public int cannonRecoil = 0;

	public boolean coaxRecoil = false; //2025 allow passenger gun to cause tank recoil animation
	...
	protected void read(String[] split, TypeFile file){
		...
		try{ 
			...
			if(FMLCommonHandler.instance().getSide().isClient() && split[0].equals("Model"))
			{
				...
				modelShitty = FlansMod.proxy.loadModel(split[1], shortName, ModelDriveable.class); //shitty model is same as normal model unless specified in next config
			}
			if(FMLCommonHandler.instance().getSide().isClient() && split[0].equals("ModelShitty")) //2025 level of detail experiment lod
				modelShitty = FlansMod.proxy.loadModel(split[1], shortName, ModelDriveable.class);
			//else modelShitty = model; //2025 if no config, just use same model at long range
			 if(split[0].equals("VehicleGunReloadTick")) //2025
				reloadSoundTick = Integer.parseInt(split[1]);
			else if(split[0].equals("Texture"))
				texture = split[1];
			else if(split[0].equals("TextureShitty"))
				textureShitty = split[1];
			...
			if(split[0].equals("maxLockOnAngle"))
				maxLockOnAngle = Float.parseFloat(split[1]); //2024 for plane guiding the missiles
			
			if(split[0].equals("cannonRecoil")) //2024 1st person vehicle recoil
				cannonRecoil = Integer.parseInt(split[1]); //default is 0, max is 599. any higher and it auto lowers to 500
			
            //2025 weapon auto door
            if(split[0].equals("primaryDoor"))
            	primaryDoor = Boolean.parseBoolean(split[1]);
            if(split[0].equals("secondaryDoor"))
            	secondaryDoor = Boolean.parseBoolean(split[1]);
			...
			else if(split[0].equals("coaxRecoil")) //2025 allow drivergun to cause tankr ecoil animation
				coaxRecoil = Boolean.parseBoolean(split[1]);
			...
			{
				tier = Float.parseFloat(split[1]);
				expValue = (7+tier)*135 - 992;  //automatically calculate a base exp value based on tier. ranges roghly from 5 at tier-6 to 1500 at tier 6   if you use the explevel config, then you can overwrite it
				if (expValue < 5)
					expValue = 5;
				repairCost = expValue; //I assume vehicle price is about 10x exp value, so 1/10 vehicle cost per repair is oppressive enough to prevent auto-repair exp farming
			}
			else if(split[0].equals("expValue"))
				expValue = Float.parseFloat(split[1]);
			else if(split[0].equals("repairCost"))
				repairCost = Float.parseFloat(split[1]);
            ...
            //2024 improved version of missile display. usually for plane wings but hey let tanks do it too
            if(split[0].equals("missilePrimaryAuto"))
            	missilePrimaryAuto = Boolean.parseBoolean(split[1]);
            if(split[0].equals("missileSecondaryAuto"))
            	missileSecondaryAuto = Boolean.parseBoolean(split[1]);
            
            //2024 turret toss config
            if(split[0].equals("turretTossTrue"))
            	turretTossTrue = Boolean.parseBoolean(split[1]);
			if(split[0].equals("turretGrenade"))
				turretGrenade = split[1];
