...
public class SRadarPacket implements IMessage {
	...
	public void fromBytes(ByteBuf buf) {
		...
		for(int i = 0; i < blips.length; i++) {
			blips[i] = new Blip(buf.readFloat(), buf.readFloat(), buf.readFloat(), buf.readFloat(), buf.readFloat(), buf.readShort(), buf.readFloat());
		}
	}
	...
	public void toBytes(ByteBuf buf) {
		...
		for(int i = 0; i < blips.length; i++) {
			...
			buf.writeFloat(blips[i].Distance);
		}
	}
		...
		public IMessage onMessage(SRadarPacket m, MessageContext ctx) {
			...
			if(m.blips != null)
				for(Blip blip : m.blips)
					MainRegistry.proxy.addBlip(blip.x, blip.y, blip.z, blip.posX, blip.posZ, blip.type, blip.Distance);
