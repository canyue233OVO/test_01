...
public class ClientProxy extends CommonProxy
{
	...
	public void doTutorialStuff(EntityPlayer player, EntityDriveable entityType)
	{
		//if (!FlansModClient.doneTutorial)
		{
			...
			player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.eject.getKeyCode()) + " to eject passengers and enable driver-controlled passenger guns"));
			player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.openBorders.getKeyCode()) + " to allow passengers and passenger-controlled guns again"));
			...
			if (entityType instanceof EntityPlane)
			{
				...
				if(PlaneType.getPlane(((EntityPlane)entityType).driveableType).mode == EnumPlaneMode.VTOL)
					...
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.modeKey.getKeyCode()) + " to toggle landing flaps"));
				if(PlaneType.getPlane(((EntityPlane)entityType).driveableType).carrierLandable || PlaneType.getPlane(((EntityPlane)entityType).driveableType).helipadLandable)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.gunKey.getKeyCode()) + " to dock on helipad / floatplane hangar"));
			...
			if (entityType instanceof EntityVehicle)
			{
				...
				player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.controlSwitchKey.getKeyCode()) + " for mouse control mode"));  //hello boys im back!
				player.addChatComponentMessage(new ChatComponentText("Hold " + Keyboard.getKeyName(KeyInputHandler.nintendoSwitch.getKeyCode()) + " to switch seats"));
				//if(((EntityVehicle) entityType).getVehicleType().tank)
				//	player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.leftRollKey.getKeyCode()) + " to lock a tank turret / cannon"));   //not anymore june 16 2023 old mouse control is back
				//if(((EntityVehicle) entityType).getVehicleType().tank)
				...
				if(entityType.getDriveableType().hasRadar)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.artilleryMode.getKeyCode()) + " to toggle active / passive radar"));			
				if(entityType.getDriveableType().hasScope)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.downKey.getKeyCode()) + " to toggle gun optic"));
				player.addChatComponentMessage(new ChatComponentText("Hold the spacebar to brake"));
			}
		}
	}
