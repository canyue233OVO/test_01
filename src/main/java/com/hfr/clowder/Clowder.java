...
clowder.pussy(world);
    ...
    public void saveClowder(int i, NBTTagCompound nbt) {
        ...
        */
        ...
        // ===== 保存新系统数据 =====
        nbt.setInteger(i + "_politicalPoints", this.politicalPoints);
        
        if (this.populationSystem != null) {
            nbt.setTag(i + "_populationSystem", this.populationSystem.saveToNBT());
        }
        
        if (this.policySystem != null) {
            nbt.setTag(i + "_policySystem", this.policySystem.saveToNBT());
        }
        
        if (this.decisionSystem != null) {
            nbt.setTag(i + "_decisionSystem", this.decisionSystem.saveToNBT());
        }
        
        if (this.techTreeSystem != null) {
            nbt.setTag(i + "_techTreeSystem", this.techTreeSystem.saveToNBT());
        }
        
        if (this.productionSystem != null) {
            nbt.setTag(i + "_productionSystem", this.productionSystem.saveToNBT());
        }
        
        if (this.storehouse != null) {
            nbt.setTag(i + "_storehouse", this.storehouse.saveToNBT());
        }
        
        if (this.chunkManager != null) {
            nbt.setTag(i + "_chunkManager", this.chunkManager.saveToNBT());
        }
    ...
    public static Clowder loadClowder(int i, NBTTagCompound nbt) {
        ...
        */
        ...
        // ===== 加载新系统数据 =====
        c.politicalPoints = nbt.getInteger(i + "_politicalPoints");
        
        // 初始化新系统
        c.populationSystem = new PopulationSystem();
        c.policySystem = new PolicySystem();
        c.decisionSystem = new DecisionSystem();
        c.techTreeSystem = new TechTreeSystem();
        c.productionSystem = new ProductionSystem();
        c.storehouse = new ClowderStorehouse(c.uuid);
        c.chunkManager = new ChunkManager(c.uuid);
        
        // 加载数据
        if (nbt.hasKey(i + "_populationSystem")) {
            c.populationSystem.loadFromNBT(nbt.getCompoundTag(i + "_populationSystem"));
        }
        
        if (nbt.hasKey(i + "_policySystem")) {
            c.policySystem.loadFromNBT(nbt.getCompoundTag(i + "_policySystem"));
        }
        
        if (nbt.hasKey(i + "_decisionSystem")) {
            c.decisionSystem.loadFromNBT(nbt.getCompoundTag(i + "_decisionSystem"));
        }
        
        if (nbt.hasKey(i + "_techTreeSystem")) {
            c.techTreeSystem.loadFromNBT(nbt.getCompoundTag(i + "_techTreeSystem"));
        }
        
        if (nbt.hasKey(i + "_productionSystem")) {
            c.productionSystem.loadFromNBT(nbt.getCompoundTag(i + "_productionSystem"));
        }
        
        if (nbt.hasKey(i + "_storehouse")) {
            c.storehouse.loadFromNBT(nbt.getCompoundTag(i + "_storehouse"));
        }
        
        if (nbt.hasKey(i + "_chunkManager")) {
            c.chunkManager.loadFromNBT(nbt.getCompoundTag(i + "_chunkManager"));
        }
...
//                prestige *= (float) Math.pow(0.99, clowder.getPrestige());
     ...
     * 添加政治点数
     */
    public void addPoliticalPoints(int amount) {
        politicalPoints += amount;
     ...
     * 获取派系UUID (静态方法)
     */
    public static Clowder getClowderByUUID(String uuid) {
        return getClowderFromUUID(uuid);
    }
