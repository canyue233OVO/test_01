...
public class GuiDriveableRepair extends GuiScreen 
{
    ...
    private int scroll = 0;//goldsloth scroll
	...
	protected void actionPerformed(GuiButton button)
    {
		...
		if (scroll>0) { //goldslot scroll down technology
            scroll--;
        }
    }
    
    private void updateButtons(int maxAllowableY) //allowabley is part of goldslothscroll
    {
    	int y = 43;
    	for(int i = 0; i < partsToDraw.size(); i++)
    	{
    		...
    		//y += part.health <= 0 ? 40 : 20; before goldsloth scroll
    	      if (i >= scroll) {
    ...
    {
        ScaledResolution scaledresolution = new ScaledResolution(mc, mc.displayWidth, mc.displayHeight); //lots of goldsloth scroll tech
		...
		//Render the footer
		  // don't render footer here, do it at the end according to goldsloth
		//Render the title
		...
		for(DriveablePart part : partsToDraw){
			...
			if(part.armor > 1)
			drawString(fontRendererObj, part.type.getName() + ": " + (int)(part.armor) + "mm", guiOriginX + 10, guiOriginY + y + 6, 0xffffff);
	...
	protected void mouseClicked(int i, int j, int k)
    {
		...
		if(m > 185 && m < 195 && n > 5 && n < 15) {
		 ...
		 } else if (m > 189 && m < 199 && n > 32 && n < 42 && scroll > 0) {  //more goldsloth scroll
	            scroll--;
